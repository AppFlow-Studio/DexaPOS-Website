 ...
 */
export async function GetLocationMenus(locationId: string): Promise<MenuWithLocationStatus[]> {
    if (!locationId) {
        return []
    }
...
export async function SyncMenuToAllLocations(menuId: string) {
    ...
    const existingLocationIds = new Set((existingRecords || []).map(r => r.location_id))
 ...
 */
export async function DeleteLocationMenu(
    locationId: string,
    menuId: string
) {
    if (!locationId || !menuId) {
