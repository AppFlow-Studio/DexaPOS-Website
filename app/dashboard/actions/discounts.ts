...
function buildPayload(input: DiscountFormInput, merchantId: string) {
    ...
    return {
        ...
        max_discount_amount: parsed.discount_type === 'percentage' ? parsed.max_discount_amount ?? null : null,
        ...
        applicable_days: parsed.applicable_days?.length ? parsed.applicable_days : defaultApplicableDays,
        ...
        exclude_categories: parsed.exclude_categories?.length ? parsed.exclude_categories : null,
        applies_to_categories: parsed.applies_to_categories?.length ? parsed.applies_to_categories : null,
...
export async function listDiscounts(filters: DiscountListFilters = {}) {
    try {
        ...
        const { data, error } = await query.order(sortBy, { ascending }).order('created_at', { ascending: false })
        ...
        return { success: false, error: error instanceof Error ? error.message : 'Failed to fetch discounts' }
    }
}
...
export async function getDiscountById(discountId: string) {
    try {
        ...
        const menuItemIds =
            (data as unknown as { menu_item_discounts?: { menu_item_id: string }[] }).menu_item_discounts?.map(
                (row) => row.menu_item_id,
            ) || []

        return { success: true, data: { ...(data as Discount), menu_item_ids: menuItemIds } }
    } catch (error) {
        console.error('[getDiscountById] error', error)
        return { success: false, error: error instanceof Error ? error.message : 'Failed to load discount' }
    }
}

async function upsertMenuItemLinks(discountId: string, merchantId: string, menuItemIds?: string[]) {
    const supabase = createServerSupabaseClient()

    // Clear existing links first
    await supabase.from('menu_item_discounts').delete().eq('discount_id', discountId)

    if (!menuItemIds || menuItemIds.length === 0) return

    await supabase.from('menu_item_discounts').insert(
...
export async function createDiscount(input: DiscountFormInput): Promise<MutationResult<Discount>> {
    try {
        const supabase = createServerSupabaseClient()
        const merchantId = await getMerchantIdFromSession()
...
export async function updateDiscount(discountId: string, input: DiscountFormInput): Promise<MutationResult<Discount>> {
    try {
        const supabase = createServerSupabaseClient()
        const merchantId = await getMerchantIdFromSession()
        const payload = buildPayload(input, merchantId)
...
export async function toggleDiscountActive(discountId: string, isActive: boolean): Promise<MutationResult<null>> {
    try {
        const supabase = createServerSupabaseClient()
        const merchantId = await getMerchantIdFromSession()
...
export async function bulkUpdateDiscountStatus(discountIds: string[], isActive: boolean): Promise<MutationResult<number>> {
    try {
...
export async function deleteDiscount(
    ...
    try {
        ...
        const { error } = await supabase.from('discounts').delete().eq('id', discountId).eq('merchant_id', merchantId)
        if (error) throw error
...
export async function bulkDeleteDiscounts(
    ...
    try {
        ...
        await supabase.from('menu_item_discounts').delete().in('discount_id', discountIds)
        const { error, count } = await supabase
            .from('discounts')
            .delete()
            .in('id', discountIds)
        ...
        return { success: false, error: error instanceof Error ? error.message : 'Failed to delete discounts' }
    }
}

export async function getDiscountUsage(discountId: string): Promise<MutationResult<DiscountUsageStats>> {
    try {
        const supabase = createServerSupabaseClient()
        const merchantId = await getMerchantIdFromSession()
        ...
        return { success: true, data: { usage_count: count || 0, last_used_at: null } }
    } catch (error) {
        console.error('[getDiscountUsage] error', error)
        return { success: false, error: error instanceof Error ? error.message : 'Failed to load usage' }
    }
}
...
export async function listCategoriesForMerchant() {
    try {
        ...
        return { success: false, error: error instanceof Error ? error.message : 'Failed to load categories' }
    }
}
...
export async function listMenuItemsForMerchant() {
    try {
        ...
        return { success: false, error: error instanceof Error ? error.message : 'Failed to load menu items' }
    }
}
