...
export async function acceptPosInvite(token: string, pin: string) {
    ...
    const displayName = `${invite.first_name} ${invite.last_name?.charAt(0) || ''}.`
...
export async function createCustomRole(params: CreateCustomRoleParams) {
    ...
    const roleCode = `${params.merchantId.substring(0, 8)}.${params.name.toLowerCase().replace(/\s+/g, '_')}`
    ...
    try {
        ...
        const { data: role, error: roleError } = await supabase
            ...
            .insert({
                ...
                requires_clerk_account: params.canAccessDashboard ?? baseRole.requires_clerk_account,
                can_access_dashboard: params.canAccessDashboard ?? baseRole.can_access_dashboard,
...
async function sendStaffInviteNotification(params: {
    ...
    const message = `Hi ${params.firstName}! You've been invited to join ${merchant?.name || 'the team'} on DEXA POS. Set up your PIN here: ${setupUrl}`
