...
export async function createStationDevice(input: CreateStationDeviceInput) {
  try {
    ...
    if (stationError || !station) {
      console.error('[createStationDevice] Error getting station:', stationError)
 ...
 */
export async function updateStationDevice(deviceId: string, input: UpdateStationDeviceInput) {
  try {
    const supabase = createServerSupabaseClient()
    ...
    if (input.device_name !== undefined) updateData.device_name = input.device_name
    if (input.device_model !== undefined) updateData.device_model = input.device_model
    ...
    if (input.connection_address !== undefined) updateData.connection_address = input.connection_address
    if (input.connection_port !== undefined) updateData.connection_port = input.connection_port
    if (input.printer_width !== undefined) updateData.printer_width = input.printer_width
    if (input.printer_dpi !== undefined) updateData.printer_dpi = input.printer_dpi
    if (input.auto_cut !== undefined) updateData.auto_cut = input.auto_cut
    if (input.open_cash_drawer !== undefined) updateData.open_cash_drawer = input.open_cash_drawer
  ...
  try {
    ...
    await supabase
      ...
      .update({
        ...
        last_seen_at: simulatedSuccess ? new Date().toISOString() : device.last_seen_at,
