...
export default function MerchantDashboardPage() {
    ...
    const { data: analytics7Days, isLoading: analyticsLoading7Days } = useOrderAnalytics(last7Days, now)
    const { data: analytics30Days, isLoading: analyticsLoading30Days } = useOrderAnalytics(last30Days, now)
    ...
    const growth = useMemo(() => {
        ...
        const currentPeriod = analytics7Days.salesByDate.reduce((sum, item) => sum + item.sales, 0)
        ...
        const last7DaysSales = last14Days.slice(-7).reduce((sum, item) => sum + item.sales, 0)
        const previous7DaysSales = last14Days.slice(0, 7).reduce((sum, item) => sum + item.sales, 0)
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <CardContent>
                        {analyticsLoading7Days ? (
                            ...
                            <div className="text-2xl font-bold">
                                {formatCurrency(
                                    analytics7Days?.salesByDate?.reduce((sum, item) => sum + item.sales, 0) || 0
                                )}
                            </div>
                        )}
                ...
                <Card className="transition-all hover:shadow-md animate-in fade-in slide-in-from-bottom-4 duration-300" style={{ animationDelay: '100ms' }}>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">
                            {isAllLocations ? `${acceptingOrdersCount} accepting orders` : staffMembers?.length ? 'At this location' : 'No team members'}
                        </p>
                    </CardContent>
                </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {analyticsLoading7Days ? (
                                ...
                                <div className="space-y-3">
                                    {Object.entries(analytics7Days.orderTypeBreakdown).map(([type, count]) => {
                                        if (count === 0) return null
                                        const total = Object.values(analytics7Days.orderTypeBreakdown).reduce((sum, c) => sum + c, 0)
                                    ...
                                    {Object.values(analytics7Days.orderTypeBreakdown).every(c => c === 0) && (
            ...
            {/* Best Selling Items */}
            {analytics7Days?.bestSellingItems && analytics7Days.bestSellingItems.length > 0 && (
                <Card>
                    <CardHeader>
                        <CardTitle className="text-base">Best Selling Items</CardTitle>
                        <CardDescription>Top items by revenue (last 7 days)</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-2">
                            {analytics7Days.bestSellingItems.slice(0, 5).map((item, index) => (
                            ...
                            {ordersList.slice(0, 5).map((order) => (
                                ...
                                >
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                            <span className="font-medium">Order #{order.display_number || order.order_number}</span>
