...
import {
  useHQCashDrawerSessions,
  useHQCashDrawerOperations,
  useHQNoSaleOperations,
  useHQCashDrawerSummaryStats,
  useHQVarianceTrend,
} from "../../hooks/useHQCashDrawerAnalytics";
import type { HQCashDrawerSession, HQCashDrawerFilters, HQVarianceTrendPoint } from "../../actions/hq-platform/cash-drawer-analytics";
import { CashDrawerFilterBar } from "./CashDrawerFilterBar";
import { getPlatformLocations, type PlatformLocation } from "@/app/manage/actions/hq-platform/transactions";
...
function SortIcon({ active, asc }: { active: boolean; asc: boolean }) {
  ...
  return asc ? <ArrowUp className="inline ml-1 h-3 w-3" /> : <ArrowDown className="inline ml-1 h-3 w-3" />;
}
...
const HQ_SESSION_COLUMNS: [HQSortKey, string][] = [
  ["business_date", "Date"],
...
const SESSION_COLSPAN = 1 + HQ_SESSION_COLUMNS.length;
...
function SessionsTab({
  ...
  const handleSort = (key: HQSortKey) => {
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `platform-cash-drawer-sessions-${format(dateFrom, "yyyy-MM-dd")}-${format(dateTo, "yyyy-MM-dd")}.csv`;
    a.click();
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50">
              ...
              {HQ_SESSION_COLUMNS.map(([key, label]) => (
                <TableHead
                  key={key}
          ...
          <TableBody>
            {isLoading
              ? skeletonRows.map((_, i) => (
                  <TableRow key={i}>
                    {Array.from({ length: SESSION_COLSPAN }).map((_, j) => (
              ...
              ? (
                <TableRow>
                  <TableCell colSpan={SESSION_COLSPAN} className="p-0">
                    <EmptyState
                      title="No sessions found"
                      description="Try adjusting the date range, filters, or status"
                    />
                  </TableCell>
                </TableRow>
              )
              : paginated.map((session) => (
                  <React.Fragment key={session.id}>
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground">{session.location_name}</TableCell>
...
function ExpandedOperations({ sessionId }: { sessionId: string }) {
  const { data: ops = [], isLoading } = useHQCashDrawerOperations(sessionId);
...
function NoSaleTab({
  ...
  const drawers = useMemo(() => {
    const set = new Set(
      ops.map((op) => {
        const base = op.drawer_name ?? "";
        if (isMultiMerchant && op.merchant_name) return `${op.merchant_name} / ${base}`;
        return base;
      })
    );
  ...
  const filteredOps = useMemo(() => {
    return ops.filter((op) => {
      ...
      if (drawerFilter !== "all") {
        const qualifiedDrawer =
          isMultiMerchant && op.merchant_name
            ? `${op.merchant_name} / ${op.drawer_name ?? ""}`
            : (op.drawer_name ?? "");
        if (qualifiedDrawer !== drawerFilter) return false;
      }
      return true;
    });
  }, [ops, employeeFilter, drawerFilter, isMultiMerchant]);
  ...
  function exportCSV() {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `platform-no-sale-audit-${format(dateFrom, "yyyy-MM-dd")}-${format(dateTo, "yyyy-MM-dd")}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }

  const skeletonRows = Array.from({ length: 5 });
  const noSaleColSpan = isMultiMerchant ? 6 : 5;
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                {isMultiMerchant && <TableHead>Merchant</TableHead>}
            ...
            <TableBody>
              {isLoading
                ? skeletonRows.map((_, i) => (
                    <TableRow key={i}>
                      {Array.from({ length: noSaleColSpan }).map((_, j) => (
                ...
                : filteredOps.map((op) => (
                    <TableRow key={op.id}>
                      ...
                      {isMultiMerchant && (
                        <TableCell className="text-muted-foreground">{op.merchant_name ?? "—"}</TableCell>
                      )}
                      <TableCell>
                        <span
                          className={
...
function VarianceTrendsTab({
  ...
  const lineKey = (point: HQVarianceTrendPoint) =>
    isMultiMerchant
      ? `${point.merchant_name} / ${point.drawer_name}`
      : point.drawer_name;

  const lineKeys = useMemo(() => {
    const set = new Set(trend.map(lineKey));
    return Array.from(set);
  }, [trend, isMultiMerchant]);

  const chartData = useMemo(() => {
    const byDate = new Map<string, Record<string, number | null>>();
    trend.forEach((t) => {
      if (!byDate.has(t.business_date)) byDate.set(t.business_date, { date: t.business_date });
      byDate.get(t.business_date)![lineKey(t)] = t.variance;
    });
  ...
  const drawerStats = useMemo(() => {
    return lineKeys.map((key) => {
      const points = trend.filter(
        (t) => lineKey(t) === key && t.variance != null
      );
      if (points.length === 0) return { key, avg: 0, worst: null as typeof points[0] | null, best: null as typeof points[0] | null, total: 0 };
      ...
      return { key, avg, worst, best, total };
    });
  }, [trend, lineKeys]);

  const sortedStats = useMemo(
    () => [...drawerStats].sort((a, b) => Math.abs(b.total) - Math.abs(a.total)),
    [drawerStats]
  );
  const visibleStats = showAllStats ? sortedStats : sortedStats.slice(0, 20);
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  <TableHead>{isMultiMerchant ? "Merchant / Drawer" : "Drawer"}</TableHead>
              ...
              <TableBody>
                {visibleStats.map((ds) => (
                  <TableRow key={ds.key}>
                    <TableCell className="font-medium">{ds.key}</TableCell>
                    <TableCell className="tabular-nums">
            ...
            {sortedStats.length > 20 && !showAllStats && (
              <div className="mt-3 flex justify-center">
                ...
                >
                  Show all {sortedStats.length} drawers
                </Button>
              </div>
            )}
          </CardContent>
...
// ─── Location-only Filter Bar (for merchant-scoped view) ──────────────────────

function LocationOnlyFilterBar({
  merchantId,
  filters,
  onFiltersChange,
}: {
  ...
  useEffect(() => {
    getPlatformLocations([merchantId]).then(setLocations);
  }, [merchantId]);
  ...
  const hasFilter = selectedLocationCount > 0;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {hasFilter && (
        <Button variant="ghost" size="sm" className="h-8 gap-1 text-muted-foreground" onClick={clearLocations}>
...
export function CashDrawerAnalytics({ lockedMerchantId }: { lockedMerchantId?: string }) {
  ...
  const [filters, setFilters] = useState<HQCashDrawerFilters>(
  ...
  function handleFiltersChange(next: HQCashDrawerFilters) {
    if (lockedMerchantId) {
      // Keep merchant locked, only allow location changes
      setFilters({ ...next, merchantIds: [lockedMerchantId] });
    } else {
      setFilters(next);
    }
  }

  if (!permLoading && !hasPermission("analytics.view")) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-3 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          You don&apos;t have permission to view cash drawer reports.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      {/* Show breadcrumb only on the standalone page (not when embedded in merchant) */}
      {!lockedMerchantId && (
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          {!lockedMerchantId && (
            <>
              <h2 className="text-2xl font-bold tracking-tight">Platform Cash Drawer Reports</h2>
              <p className="text-muted-foreground">
                Cross-merchant session history, variance trends, and No Sale audit logs
              </p>
            </>
          )}
          {lockedMerchantId && (
            <p className="text-sm text-muted-foreground">
              Session history, variance trends, and No Sale audit logs for this merchant
            </p>
          )}
        </div>
        <DateRangePicker
      ...
      {/* Filter bar */}
      {lockedMerchantId ? (
        <LocationOnlyFilterBar
          merchantId={lockedMerchantId}
          filters={filters}
          onFiltersChange={handleFiltersChange}
        />
      ) : (
        <CashDrawerFilterBar filters={filters} onFiltersChange={setFilters} />
      )}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="sessions" className="mt-6">
          <SessionsTab filters={filters} dateFrom={dateRange.from} dateTo={dateRange.to} />
        </TabsContent>
        <TabsContent value="no-sale" className="mt-6">
          <NoSaleTab filters={filters} dateFrom={dateRange.from} dateTo={dateRange.to} />
        </TabsContent>
        <TabsContent value="variance" className="mt-6">
          <VarianceTrendsTab filters={filters} dateFrom={dateRange.from} dateTo={dateRange.to} />
        </TabsContent>
      </Tabs>
    </div>
  );
