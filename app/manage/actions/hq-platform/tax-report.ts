...
export interface HQMerchantOption {
  id: string;
  name: string;
}
...
export async function GetHQMerchantsForTax(): Promise<{
  data?: HQMerchantOption[];
  error?: string;
}> {
  try {
  ...
  if (error) {
    console.error("[HQTaxReport] merchants error:", error);
...
export async function GetHQTaxSummary(
  merchantId: string,
  ...
  if (ordersError) {
    console.error("[HQTaxReport] summary error:", ordersError);
...
export async function GetHQTaxBreakdown(
  merchantId: string,
  locationId: string | null,
  dateFrom: Date,
  dateTo: Date,
  ...
  if (error) {
    console.error("[HQTaxReport] breakdown error:", error);
    return { error: error.message };
  }
...
export async function GetHQTaxByCategory(
  merchantId: string,
  locationId: string | null,
  dateFrom: Date,
  dateTo: Date
...
export async function GetHQTaxByLocation(
  merchantId: string,
  dateFrom: Date,
  dateTo: Date
