...
export async function getGPVConcentration(days: number = 30): Promise<GPVConcentrationData> {
  ...
  if (whaleIds.length > 0) {
    const [{ data: merchantNames }, { data: locationRows }] = await Promise.all([
      supabase.from('merchants').select('id, business_name, account_manager').in('id', whaleIds),
...
export async function getFleetHealth(): Promise<FleetHealthData> {
  ...
  const merchantNameMap = new Map(merchantRows?.map(m => [m.id, m.business_name || 'Unknown']) || [])
...
export async function getMerchantOnboardingFunnel(): Promise<MerchantOnboardingFunnelData> {
  ...
  const statusCounts = {
    ...
    suspended: merchants.filter(m => m.status === 'suspended' || m.status === 'churned').length,
  }
  ...
  const stuckMerchantList = merchants.filter(m => m.status === 'onboarding' && new Date(m.created_at) < fourteenDaysAgo)
  ...
  merchants.forEach(m => {
    ...
    entry.newCount += 1
    if (m.status === 'active') entry.activeCount += 1
  })
...
export interface NeverActivatedMerchant {
  ...
  hasStaff: boolean
  hasDevice: boolean
  onboardingScore: number // 0-4 checklist items
}
...
export async function getMerchantActivationTimeline(): Promise<MerchantActivationData> {
  ...
  const [merchantsRes, firstOrdersRes] = await Promise.all([
    supabase.from('merchants').select('id, business_name, created_at'),
  ...
  merchants.forEach(m => {
    ...
    if (new Date(firstOrderAt) >= startOfMonth) activatedThisMonth += 1
  })
  ...
  const [menuRes, staffRes, stationsRes] = naIds.length > 0
    ? await Promise.all([
  ...
  const neverActivated: NeverActivatedMerchant[] = neverActivatedRaw.slice(0, 50).map(m => {
    ...
    return {
      ...
      hasMenu, hasStaff, hasDevice,
      onboardingScore: [hasMenu, hasStaff, hasDevice, false].filter(Boolean).length + 1, // +1 for having account
    }
  }).sort((a, b) => b.daysSinceCreation - a.daysSinceCreation)

  return {
    histogram: buckets, neverActivated, avgDaysToActivate, medianDaysToActivate,
    activatedThisMonth, totalMerchantsWithOrders: firstOrderMap.size,
  }
}
...
export async function getPaymentMethodMix(days: number = 30): Promise<PaymentMethodMixData> {
  ...
  const { data: merchantRows } = mids.length > 0
    ...
    : { data: [] }
  const nameMap = new Map((merchantRows || []).map(m => [m.id, m.business_name || 'Unknown']))
  ...
  const [dualPricingLocationsRes, trendPaymentsRes] = await Promise.all([
    allMerchantIds.length > 0
      ...
      : Promise.resolve({ data: [] }),
    supabase.from('order_payments').select('payment_method, amount, created_at')
...
export async function getVoidRefundIntelligence(days: number = 30): Promise<VoidRefundData> {
  ...
  const { data: staffRows } = staffIds.length > 0
    ? await supabase.from('staff_profiles').select('id, first_name, last_name, merchant_id').in('id', staffIds)
    : { data: [] }
  const staffNameMap = new Map((staffRows || []).map(s => [s.id, `${s.first_name || ''} ${s.last_name || ''}`.trim() || 'Unknown']))
...
export async function getDiscountUsageAnalysis(days: number = 30): Promise<DiscountAbuseData> {
  ...
  const [discountRes, merchantRes] = await Promise.all([
    discountIds.length > 0
      ...
      : Promise.resolve({ data: [] }),
    allMids.length > 0
      ? supabase.from('merchants').select('id, business_name').in('id', allMids)
      : Promise.resolve({ data: [] }),
  ])
  ...
  const nameMap = new Map((merchantRes.data || []).map(m => [m.id, m.business_name || 'Unknown']))
...
export async function getStaffLaborAnalytics(days: number = 30): Promise<StaffLaborData> {
  ...
  const merchantNameMap = new Map(merchants.map(m => [m.id, m.business_name]))
...
export async function getMultiLocationComparison(days: number = 30): Promise<MultiLocationComparisonData> {
  ...
  const [currentRes, prevRes, locationsRes] = await Promise.all([
    supabase
      .from('orders')
      .select('location_id, merchant_id, total_amount, status')
      .gte('created_at', periodStart.toISOString()),
    supabase
      .from('orders')
  ...
  const metrics: LocationMetrics[] = locations
    .filter(l => locMap.has(l.id))
    .map(loc => {
      const s = locMap.get(loc.id)!
