-- Rollback: restore the previous (incorrect cents-based) get_admin_settlement_batches.
-- Provided only so a downgrade can replay the legacy behavior; do not run unless
-- explicitly reverting 20260505120000_fix_admin_settlement_batches_dollars.
...
RETURNS TABLE(
  id uuid, batch_id text, merchant_id uuid, merchant_name text,
  location_id uuid, location_name text, business_date date,
  opened_at timestamp with time zone, closed_at timestamp with time zone,
  settlement_date date, funded_date date, transaction_count integer,
  sales_count integer, refund_count integer, void_count integer,
  gross_amount numeric, tip_amount numeric, refund_amount numeric,
  net_deposit numeric, status text, linked_payment_count bigint,
  linked_payment_amount numeric, discrepancy_amount numeric,
...
BEGIN
  ...
  WITH scoped_batches AS (
    SELECT sb.id, sb.batch_id, sb.merchant_id,
  ...
  SELECT
    b.id, b.batch_id::text, b.merchant_id, b.merchant_name::text,
    b.location_id, b.location_name::text, b.business_date,
    b.opened_at, b.closed_at, b.settlement_date, b.funded_date,
  ...
  LEFT JOIN LATERAL (
    SELECT COUNT(*)::bigint AS linked_payment_count,
      COALESCE(SUM(
        ...
        END
      ), 0)::numeric AS linked_payment_amount
