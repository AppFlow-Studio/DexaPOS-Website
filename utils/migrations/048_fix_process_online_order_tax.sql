-- ============================================================================
-- Migration 048: Fix process_online_order tax calculation
-- ============================================================================
-- Problem: The RPC was only looking for tax_category = 'default', but
--          dashboard-configured rates use tax_category = 'standard'.
--          When p_tax = 0 arrives from the edge function, the order was
--          stored with $0 tax even though a valid rate exists.
--
-- Fix:
--   1. Remove the tax_category filter — pick the first active rate for the
--      location regardless of category name.
--   2. If p_tax = 0 but a rate IS found, recalculate p_tax and p_total from
--      the database rate (self-healing: fixes edge function failures silently).
-- ============================================================================
...
COMMENT ON FUNCTION public.process_online_order IS
  'Universal online order processor. v3: removed tax_category filter (picks any active rate), added self-healing tax recalculation when p_tax=0.';
