-- ============================================================================
-- Migration 048: Fix unsafe INTEGER casts in process_online_order
-- ============================================================================
-- The function was casting JSONB quantity fields directly to INTEGER with
-- no protection against non-numeric values (e.g. 'S6', 'null', strings).
-- This migration adds a safe_jsonb_int() helper and updates the function.
-- ============================================================================

-- ============================================================================
-- HELPER: safe_jsonb_int
-- Safely converts a JSONB value (or extracted text) to INTEGER.
-- Returns default_val for any non-numeric input instead of throwing.
-- ============================================================================
...
COMMENT ON FUNCTION public.safe_jsonb_int IS
'Safely extract an integer from a JSONB value. Returns p_default for any non-integer input (null, strings, floats, etc.).';

-- ============================================================================
-- UPDATE process_online_order to use safe_jsonb_int()
-- Only the two integer-cast lines change; everything else is identical to v2.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      -- SAFE integer cast: non-numeric values (e.g. 'S6') fall back to 1
      v_item_qty        := GREATEST(1, public.safe_jsonb_int(v_item->'quantity', 1));
      ...
      IF v_item->'modifiers' IS NOT NULL AND jsonb_array_length(v_item->'modifiers') > 0 THEN
        ...
        LOOP
          ...
          BEGIN
            ...
            -- SAFE integer cast: non-numeric values fall back to 1
            v_mod_qty   := GREATEST(1, public.safe_jsonb_int(v_modifier->'quantity', 1));
...
COMMENT ON FUNCTION public.process_online_order IS
'Universal online order processor. v3: safe integer casts for JSONB quantity fields using safe_jsonb_int() helper.';
