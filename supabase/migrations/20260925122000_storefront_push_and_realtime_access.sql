-- =============================================================================
-- Storefront order push + Realtime access for dashboard listeners
-- =============================================================================
-- Plan: Dexa-POS/docs/engineering/database/SUPABASE-CONNECTIONS-AND-STORAGE-2026-09-24.md,
-- Phase 5.2 and 5.4 (5.5 relies on the policy below).
--
-- 5.2a Storefront visitors get order status pushes from EVERY status change,
--      not only the ones the Website's own REST emitters send. Until now a
--      POS accept / ready never reached them (their postgres_changes binding
--      is blocked by orders RLS for the anon client). Same public topic, event
--      and payload as app/sites/order-actions.ts and create-online-order:
--        topic order-update:{order_id}, event status_changed, {orderId, status}
--      Only non-POS orders (storefront, kiosk, OrderOut). The watcher ignores
--      a repeated status, so overlapping REST emits are harmless.
-- 5.2b qr-session:{token} broadcasts become public (private = false): the anon
--      QR guest page listens on the public channel and received nothing, so it
--      fell back to a 5 s poll. Only that one send changes; the location
--      broadcast stays private.
-- 5.4  RLS on realtime.messages: authenticated users may receive the private
--      location broadcasts of locations they belong to (or any, for Dexa HQ):
--        location:{id}:tables, location:{id}:orders, floor-plan-{id}
...
-- -----------------------------------------------------------------------------
-- 5.2a Status push for storefront / kiosk / OrderOut orders
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.broadcast_online_order_status()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  PERFORM realtime.send(
    ...
    'status_changed',
    'order-update:' || NEW.id::text,
    false
  );
  RETURN NULL;
EXCEPTION WHEN OTHERS THEN
  -- A failed push must never fail the order update.
  RAISE WARNING 'broadcast_online_order_status failed for order %: % (%)', NEW.id, SQLERRM, SQLSTATE;
  RETURN NULL;
END;
$function$;

REVOKE ALL ON FUNCTION public.broadcast_online_order_status() FROM PUBLIC, anon, authenticated;

DROP TRIGGER IF EXISTS trg_orders_broadcast_online_status ON public.orders;
CREATE TRIGGER trg_orders_broadcast_online_status
AFTER UPDATE OF status ON public.orders
FOR EACH ROW
WHEN (OLD.status IS DISTINCT FROM NEW.status AND NEW.order_source <> 'pos')
EXECUTE FUNCTION public.broadcast_online_order_status();

-- -----------------------------------------------------------------------------
-- 5.2b QR session broadcast on the public channel
-- -----------------------------------------------------------------------------
-- Body is 20260816130000_aud10_broadcast_origin_id.sql verbatim except the
-- private flag on the qr-session send. Guard: refuse to replace a live body
-- that is not the aud10 version (it would silently drop or add changes).
DO $guard$
BEGIN
  IF position('origin_id' IN pg_get_functiondef('public.broadcast_order_changes()'::regprocedure)) = 0 THEN
    RAISE EXCEPTION 'broadcast_order_changes is not the 20260816130000 (aud10) version on this database; apply that migration first or port this change to the live body';
  END IF;
END;
$guard$;
...
-- -----------------------------------------------------------------------------
-- 5.4 Private location broadcasts for location members and Dexa HQ
-- -----------------------------------------------------------------------------
DROP POLICY IF EXISTS "location members receive location broadcasts" ON realtime.messages;
CREATE POLICY "location members receive location broadcasts"
...
USING (
  (
    (SELECT realtime.topic()) ~ '^location:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}:(tables|orders)$'
    OR (SELECT realtime.topic()) ~ '^floor-plan-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
  )
  AND (
    substring((SELECT realtime.topic()) FROM '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}')::uuid
      = ANY (public.user_location_ids())
...
-- Verify
--   5.2a  update an online_store order's status -> one realtime.messages row:
--         topic order-update:<id>, event status_changed, private false.
--   5.2b  select pg_get_functiondef('public.broadcast_order_changes()'::regprocedure) ~ '''qr_order_changed'',\s+v_qr_topic,\s+false';
--   5.4   select policyname, roles, cmd from pg_policies where schemaname = 'realtime' and tablename = 'messages';
