-- ============================================================================
-- process_payment_v17 — add the CodePay (on-terminal Intent) branch
-- ============================================================================
-- Byte-for-byte copy of 20260729122000_wave_d_process_payment_v17_extra_tender
-- _guard.sql with THREE additions, mirroring the ATOM branch:
--   1. A `codepay_transaction` extraction block funneling into the shared
--      v_dejavoo_* staging vars (card_last_four/auth/rrn/entry_mode/trans_no).
--   2. A v_terminal_id COALESCE fallback to codepay_transaction.terminalId.
--   3. A terminal_type CASE branch: codepay_transaction present => 'codepay'.
-- Requires 20260915120000_add_codepay_terminal_type.sql (enum value) applied first.
-- ============================================================================

-- Wave D — extra-tender guard message enhancement on process_payment_v17.
--
-- Re-deploys the v17 body (atom-branch base) with ONE localized change: the
-- EXISTING "already fully paid" guard now attaches the captured card last-4s
-- in the error DETAIL so the client can show "already paid on ••1234" instead
-- of a generic refusal. Everything else is byte-identical to
-- 20260723183222_process_payment_v17_atom_branch.sql.
--
-- NO CHANGE to which tenders are accepted/rejected: the threshold stays <= 0
-- (as it has since the base schema). The guard fires only when
-- payment_status='paid', so it cannot affect paying a real $0.01/penny item
-- (that happens on an UNPAID order) or an in-progress split remainder (still
-- 'partial'). Duplicate split portions are separately refused later.
--
-- ⚠️ SAME FILE LANE as the Bay Ridge P0 v16/v17 decision — sequence/merge with
-- that work before applying so the two v17 redeploys don't clobber each other.
--
-- (original header) process_payment_v17: add ATOM (on-device loopback) support.
...
BEGIN
    ...
    -- Wave D — extra-tender guard. Adds the captured card last-4s to the
    -- EXISTING already-fully-paid refusal so the client can show "already paid
    -- on ••1234". The accept/reject threshold is deliberately LEFT AT <= 0
    -- (byte-identical to prior behavior): the guard fires only when
    -- payment_status='paid', so paying a real $0.01/penny item — which happens
    -- on an UNPAID order — is never affected, and a cleanly-paid order settles
    -- at amount_due=0 anyway. This wave changes only the error message.
    IF v_order.payment_status = 'paid' AND COALESCE(v_order.amount_due, 0) <= 0 THEN
        SELECT string_agg(DISTINCT '••' || op.card_last_four, ', ')
        INTO v_captured_cards
        FROM public.order_payments op
        WHERE op.order_id = p_order_id
          AND op.status = 'captured'
          AND op.card_last_four IS NOT NULL;
        RAISE EXCEPTION 'Order is already fully paid'
            USING ERRCODE = 'P0001',
                  DETAIL = COALESCE('Already captured on ' || v_captured_cards,
    ...
    -- CodePay (on-terminal / Android Intent). Mirrors the ATOM branch:
    -- reuse the shared v_dejavoo_* staging vars so the common INSERT below
    -- persists CodePay card fields (card_last_four has NO top-level fallback).
    -- The reversal reference is the original merchant_order_no.
    IF p_terminal_response ? 'codepay_transaction' THEN
        v_has_dejavoo_transaction := true;
        v_dejavoo_reference_id := p_terminal_response->'codepay_transaction'->>'merchantOrderNo';
        v_dejavoo_transaction_number := p_terminal_response->'codepay_transaction'->>'transNo';
        v_dejavoo_auth_code := p_terminal_response->'codepay_transaction'->>'authCode';
        v_dejavoo_rrn := p_terminal_response->'codepay_transaction'->>'rrn';
        v_dejavoo_entry_mode := p_terminal_response->'codepay_transaction'->>'entryMode';
        v_dejavoo_last_four := p_terminal_response->'codepay_transaction'->>'cardLast4';
    END IF;
    ...
    v_terminal_id := COALESCE(p_terminal_id,
        ...
        (p_terminal_response->'codepay_transaction'->>'terminalId')::uuid,
    ...
    INSERT INTO public.order_payments (
        ...
        CASE WHEN v_is_cash THEN 'cash_drawer'
             ...
             WHEN p_terminal_response ? 'codepay_transaction' THEN 'codepay'
