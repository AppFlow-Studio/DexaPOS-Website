CREATE OR REPLACE FUNCTION public.manage_order_discount_v2(
  p_action text, p_order_id uuid, p_staff_id uuid,
  p_discount_id uuid DEFAULT NULL, p_discount_name text DEFAULT NULL, p_discount_type text DEFAULT NULL,
  p_discount_value numeric DEFAULT NULL, p_source text DEFAULT 'preset'::text, p_reason text DEFAULT NULL,
  p_applied_to_item_ids uuid[] DEFAULT NULL, p_approved_by_staff_id uuid DEFAULT NULL,
  p_order_discount_id uuid DEFAULT NULL, p_void_reason text DEFAULT NULL,
  p_idempotency_key UUID DEFAULT NULL
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public', 'pg_temp'
AS $function$
DECLARE
  v_cached JSONB; v_result JSONB; v_order RECORD; v_discount RECORD;
  v_is_preset_discount BOOLEAN := false; v_order_discount_id UUID; v_calculated_amount NUMERIC := 0;
  v_pre_discount_subtotal NUMERIC; v_applicable_subtotal NUMERIC; v_calculated_cash_amount NUMERIC := 0;
  v_pre_discount_cash_subtotal NUMERIC; v_applicable_cash_subtotal NUMERIC; v_discount_type TEXT;
  v_discount_value NUMERIC; v_discount_name TEXT; v_requires_approval BOOLEAN := false;
  v_max_discount_amount NUMERIC; v_is_stackable BOOLEAN := true; v_has_non_stackable BOOLEAN := false;
  v_item RECORD; v_item_discount_amount NUMERIC; v_item_proportion NUMERIC;
  v_distributed_total NUMERIC := 0; v_last_item_id UUID; v_affected_item_ids UUID[] := '{}';
  v_total_discount_amount NUMERIC := 0; v_new_card_subtotal NUMERIC; v_new_cash_subtotal NUMERIC;
  v_new_card_tax NUMERIC; v_new_cash_tax NUMERIC; v_new_card_total NUMERIC; v_new_cash_total NUMERIC;
  v_new_amount_due NUMERIC; v_new_cash_amount_due NUMERIC; v_tax_rate NUMERIC;
  v_original_card_subtotal NUMERIC; v_original_cash_subtotal NUMERIC; v_voided_discount RECORD;
...
BEGIN
  ...
  IF v_order.payment_status = 'paid' THEN RETURN jsonb_build_object('success', false, 'error', 'Cannot modify discounts on a paid order'); END IF;
  IF v_order.voided_at IS NOT NULL THEN RETURN jsonb_build_object('success', false, 'error', 'Cannot modify discounts on a voided order'); END IF;
  ...
  CASE p_action
  WHEN 'apply' THEN
    IF p_discount_name IS NULL OR p_discount_name = '' THEN RETURN jsonb_build_object('success', false, 'error', 'discount_name is required'); END IF;
    IF p_discount_type IS NULL AND p_discount_id IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'Either discount_id or discount_type must be provided'); END IF;

    IF p_discount_id IS NOT NULL THEN
      SELECT * INTO v_discount FROM public.discounts WHERE id = p_discount_id AND merchant_id = v_order.merchant_id AND is_active = true;
      IF NOT FOUND THEN RETURN jsonb_build_object('success', false, 'error', 'Discount not found or inactive'); END IF;
      v_is_preset_discount := true;
      v_discount_type := v_discount.discount_type; v_discount_value := v_discount.discount_value;
      ...
      IF v_discount.start_date IS NOT NULL AND v_discount.start_date > CURRENT_DATE THEN RETURN jsonb_build_object('success', false, 'error', 'Discount has not started yet'); END IF;
      IF v_discount.end_date IS NOT NULL AND v_discount.end_date < CURRENT_DATE THEN RETURN jsonb_build_object('success', false, 'error', 'Discount has expired'); END IF;
      IF v_discount.applicable_days IS NOT NULL AND NOT (EXTRACT(DOW FROM CURRENT_TIMESTAMP)::integer = ANY(v_discount.applicable_days)) THEN
      ...
      IF v_discount.applicable_hours_start IS NOT NULL AND v_discount.applicable_hours_end IS NOT NULL THEN
        ...
        BEGIN
          IF v_discount.applicable_hours_start > v_discount.applicable_hours_end THEN
            v_in_window := v_current_time >= v_discount.applicable_hours_start OR v_current_time <= v_discount.applicable_hours_end;
          ELSE
            v_in_window := v_current_time >= v_discount.applicable_hours_start AND v_current_time <= v_discount.applicable_hours_end;
          END IF;
          IF NOT v_in_window THEN
            RETURN jsonb_build_object('success', false, 'error',
              format('Discount only valid between %s and %s', v_discount.applicable_hours_start::text, v_discount.applicable_hours_end::text));
          END IF;
        END;
      END IF;
      ...
      SELECT EXISTS(SELECT 1 FROM public.order_discounts od LEFT JOIN public.discounts d ON d.id = od.discount_id
        WHERE od.order_id = p_order_id AND od.voided_at IS NULL AND COALESCE(d.stackable, false) = false) INTO v_has_non_stackable;
      IF v_has_non_stackable AND NOT v_is_stackable THEN RETURN jsonb_build_object('success', false, 'error', 'Cannot combine with existing non-stackable discount'); END IF;
      ...
      END IF;
    ELSE
      v_discount_type := p_discount_type; v_discount_value := p_discount_value; v_discount_name := p_discount_name;
      IF p_approved_by_staff_id IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'Custom discounts require manager approval', 'requires_approval', true); END IF;
    END IF;

    IF v_discount_value IS NULL OR v_discount_value <= 0 THEN RETURN jsonb_build_object('success', false, 'error', 'Discount value must be greater than 0'); END IF;
    IF v_discount_type = 'percentage' AND v_discount_value > 100 THEN RETURN jsonb_build_object('success', false, 'error', 'Percentage discount cannot exceed 100%'); END IF;
    ...
    IF p_applied_to_item_ids IS NOT NULL AND array_length(p_applied_to_item_ids, 1) > 0 THEN
      ...
      SELECT array_agg(id) INTO v_affected_item_ids FROM public.order_items
      WHERE id = ANY(p_applied_to_item_ids) AND order_id = p_order_id AND is_voided = false AND quantity > COALESCE(paid_quantity, 0);
    ELSE
      SELECT COALESCE(SUM((oi.quantity - COALESCE(oi.paid_quantity, 0)) * oi.unit_price), 0),
             COALESCE(SUM((oi.quantity - COALESCE(oi.paid_quantity, 0)) * COALESCE(oi.cash_price, oi.unit_price)), 0),
             array_agg(oi.id)
    ...
    IF v_is_preset_discount THEN
      IF COALESCE(v_discount.exclude_alcohol, false) THEN
        SELECT COALESCE(SUM((oi.quantity - COALESCE(oi.paid_quantity, 0)) * oi.unit_price), 0),
               ...
               array_agg(oi.id)
        INTO v_applicable_subtotal, v_applicable_cash_subtotal, v_affected_item_ids
        FROM public.order_items oi LEFT JOIN public.menu_items mi ON mi.id = oi.menu_item_id
      ...
      IF v_discount.exclude_categories IS NOT NULL THEN
        ...
        WHERE oi.order_id = p_order_id AND oi.is_voided = false
          AND oi.quantity > COALESCE(oi.paid_quantity, 0)
          AND (p_applied_to_item_ids IS NULL OR oi.id = ANY(p_applied_to_item_ids))
          AND NOT EXISTS (SELECT 1 FROM public.menu_items mi WHERE mi.id = oi.menu_item_id AND mi.category_id = ANY(v_discount.exclude_categories));
      END IF;
    ...
    IF v_applicable_subtotal <= 0 THEN RETURN jsonb_build_object('success', false, 'error', 'No applicable items for this discount'); END IF;
    IF v_is_preset_discount THEN
    ...
    v_distributed_total := 0; v_last_item_id := NULL;
    FOR v_item IN
    ...
    LOOP
      IF v_applicable_subtotal > 0 THEN v_item_proportion := v_item.item_subtotal / v_applicable_subtotal;
      ELSE v_item_proportion := 0; END IF;
      ...
      SET discount_id = p_discount_id, discount_type = v_discount_type::discount_type,
          discount_value = v_discount_value,
          discount_amount = COALESCE(discount_amount, 0) + v_item_discount_amount,
          discount_source = p_source::discount_source,
          discount_applied_by = p_staff_id, discount_approved_by = p_approved_by_staff_id,
    ...
    IF v_last_item_id IS NOT NULL AND v_distributed_total <> v_calculated_amount THEN
      ...
      SET discount_amount = discount_amount + (v_calculated_amount - v_distributed_total),
          ...
          tax_amount = ROUND(subtotal * COALESCE(tax_rate, 0) / 100, 2), updated_at = now()
      WHERE id = v_last_item_id;
    END IF;

  WHEN 'void' THEN
    IF p_order_discount_id IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'order_discount_id is required for void action'); END IF;
    ...
    IF NOT FOUND THEN RETURN jsonb_build_object('success', false, 'error', 'Discount not found or already voided'); END IF;
    ...
    UPDATE public.order_discounts SET voided_at = now(), voided_by = p_staff_id, void_reason = p_void_reason WHERE id = p_order_discount_id;
  ...
  ELSE RETURN jsonb_build_object('success', false, 'error', format('Unknown action: %s', p_action));
  END CASE;
  ...
  SELECT
    COALESCE(SUM(ROUND(oi.subtotal * (oi.quantity - COALESCE(oi.paid_quantity, 0))::NUMERIC / NULLIF(oi.quantity, 0), 2) +
                 ROUND(oi.tax_amount * (oi.quantity - COALESCE(oi.paid_quantity, 0))::NUMERIC / NULLIF(oi.quantity, 0), 2)), 0),
    COALESCE(SUM(ROUND(oi.cash_subtotal * (oi.quantity - COALESCE(oi.paid_quantity, 0))::NUMERIC / NULLIF(oi.quantity, 0), 2) +
                 ROUND(oi.cash_tax_amount * (oi.quantity - COALESCE(oi.paid_quantity, 0))::NUMERIC / NULLIF(oi.quantity, 0), 2)), 0)
  ...
  v_result := jsonb_build_object(
    ...
    'affected_items', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      FROM public.order_items oi WHERE oi.id = ANY(v_affected_item_ids) AND oi.is_voided = false),
    'active_discounts', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
