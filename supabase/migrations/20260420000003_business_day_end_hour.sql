-- =============================================================================
-- Migration: Business Day End Hour (Overnight Shift Support)
-- Date: 2026-04-20
-- =============================================================================
-- Adds a configurable "business day end hour" to locations.
-- Default 0 = midnight (no behavior change). Set to 4 for bars/restaurants
-- that operate past midnight — "Monday" becomes Mon 4 AM → Tue 4 AM.
--
-- Updates calculate_tip_distribution_v2, rebuild_employee_daily_tips,
-- and declare_cash_tips_for_shift to use business day boundaries.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_pool.distribution_method = 'equal_split' THEN
      ...
      FROM public.tip_distribution_details dd
      WHERE dd.session_id = v_session_id
        AND EXISTS (SELECT 1 FROM public.tip_pool_role_shares prs WHERE prs.tip_pool_config_id = v_pool.id AND prs.role_code = dd.role_code AND prs.is_eligible = true);
      IF v_role_count > 0 THEN
        UPDATE public.tip_distribution_details dd
    ...
    ELSIF v_pool.distribution_method = 'hours_weighted' THEN
      ...
      IF v_total_hours > 0 THEN
        UPDATE public.tip_distribution_details dd
        SET tip_pool_received = tip_pool_received + ROUND(v_pool_total * (dd.hours_worked / v_total_hours), 2)
        WHERE dd.session_id = v_session_id
          AND EXISTS (SELECT 1 FROM public.tip_pool_role_shares prs WHERE prs.tip_pool_config_id = v_pool.id AND prs.role_code = dd.role_code AND prs.is_eligible = true);
      END IF;
    ELSIF v_pool.distribution_method = 'points' THEN
      ...
      FROM public.tip_distribution_details dd
      JOIN public.tip_pool_role_shares prs ON prs.tip_pool_config_id = v_pool.id AND prs.role_code = dd.role_code AND prs.is_eligible = true
      WHERE dd.session_id = v_session_id;
      IF v_total_points > 0 THEN
        ...
        WHERE dd.session_id = v_session_id AND prs_sub.tip_pool_config_id = v_pool.id AND prs_sub.role_code = dd.role_code AND prs_sub.is_eligible = true;
      END IF;
    END IF;
  END LOOP;

  -- STEP 8: TIP-OUT RULES
  FOR v_rule IN
  ...
  LOOP
    ...
    IF v_rule.tip_out_type = 'percentage_of_sales' THEN
      ...
      IF v_receiver_count > 0 THEN
        UPDATE public.tip_distribution_details SET tip_out_received = tip_out_received + ROUND(v_tipout_total / v_receiver_count, 2)
        WHERE session_id = v_session_id AND role_code = v_rule.to_role_code;
      END IF;
    ...
    ELSIF v_rule.tip_out_type = 'flat_amount' THEN
      ...
      WHERE dd.session_id = v_session_id AND dd.role_code = v_rule.from_role_code;
      SELECT COUNT(*) INTO v_giver_count FROM public.tip_distribution_details WHERE session_id = v_session_id AND role_code = v_rule.from_role_code;
  ...
  -- STEP 9: NET TIPS
  UPDATE public.tip_distribution_details
  SET net_tips = individual_tips_earned - tip_pool_contributed + tip_pool_received - tip_out_given + tip_out_received + manual_adjustment
  WHERE session_id = v_session_id;
  ...
  UPDATE public.tip_distribution_sessions
  SET status = 'calculated', total_tips_collected = v_total_collected, total_tips_pooled = v_total_pooled,
      total_tip_outs = v_total_tipouts, total_distributed = v_total_distributed,
      rounding_adjustment = v_total_collected - v_total_distributed, config_snapshot = v_snapshot,
      calculated_at = now(), calculated_by = p_calculated_by, data_cutoff_at = v_window_end
  WHERE id = v_session_id;
...
BEGIN
  ...
  -- Business day boundaries: end_hour=4 → Mon 4 AM to Tue 4 AM
  ...
  WITH server_activity AS (
    SELECT COALESCE(o.assigned_server_id, o.created_by_staff_id) AS staff_profile_id, o.id AS order_id, o.subtotal
    FROM public.orders o
    WHERE o.location_id = p_location_id AND o.created_at >= v_day_start AND o.created_at < v_day_end
      AND o.status NOT IN ('cancelled', 'void', 'refunded')
      AND COALESCE(o.assigned_server_id, o.created_by_staff_id) IS NOT NULL
  ),
  server_charged AS (
    SELECT sa.staff_profile_id, COALESCE(SUM(op.tip_amount), 0)::NUMERIC(12,2) AS charged_tips,
      COALESCE(SUM(sa.subtotal), 0)::NUMERIC(12,2) AS gross_sales
    FROM server_activity sa JOIN public.order_payments op ON op.order_id = sa.order_id WHERE op.status = 'captured'
    GROUP BY sa.staff_profile_id
  ),
  shift_totals AS (
    SELECT ss.staff_profile_id,
      ...
      COALESCE(SUM(ss.declared_cash_tips), 0)::NUMERIC(12,2) AS cash_tips_declared
    FROM public.staff_shifts ss
    WHERE ss.location_id = p_location_id AND ss.clock_in_time >= v_day_start AND ss.clock_in_time < v_day_end
    GROUP BY ss.staff_profile_id
  ),
  combined AS (
  ...
  INSERT INTO public.employee_daily_tips (
    staff_profile_id, merchant_id, location_id, shift_date, charged_tips, gross_sales, hours_worked, cash_tips_declared, total_tips
  )
  SELECT c.staff_profile_id, v_merchant_id, p_location_id, p_shift_date,
    c.charged_tips, c.gross_sales, c.hours_worked, c.cash_tips_declared, c.charged_tips + c.cash_tips_declared
  FROM combined c WHERE c.staff_profile_id IS NOT NULL
  ON CONFLICT (staff_profile_id, location_id, shift_date) DO UPDATE
    SET charged_tips = EXCLUDED.charged_tips, gross_sales = EXCLUDED.gross_sales,
        hours_worked = EXCLUDED.hours_worked, cash_tips_declared = EXCLUDED.cash_tips_declared,
        total_tips = EXCLUDED.charged_tips + EXCLUDED.cash_tips_declared, updated_at = now()
...
BEGIN
  IF p_amount < 0 THEN
    RAISE EXCEPTION 'Declared cash tips cannot be negative' USING ERRCODE = 'check_violation';
  END IF;

  SELECT * INTO v_shift FROM public.staff_shifts WHERE id = p_shift_id FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Shift % not found', p_shift_id USING ERRCODE = 'no_data_found';
  END IF;
  ...
  SELECT sp.id INTO v_caller_sp FROM public.staff_profiles sp WHERE sp.user_id = (auth.jwt() ->> 'sub');
  ...
  IF NOT v_is_manager AND v_caller_sp IS DISTINCT FROM v_shift.staff_profile_id THEN
    RAISE EXCEPTION 'Can only declare tips for your own shift' USING ERRCODE = 'insufficient_privilege';
  END IF;

  UPDATE public.staff_shifts
  SET declared_cash_tips = p_amount, tips_declared_at = now(), updated_at = now()
  WHERE id = p_shift_id;
