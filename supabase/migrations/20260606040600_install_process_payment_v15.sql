-- Installs process_payment_v15 on prod.
--
-- Prod tops out at v13 (which delegates to v14 internally). v15 is a full
-- reimplementation — bigger fixes to closing-item-balance per
-- `20260603194300_process_payment_v15_closing_item_balance` in the POS repo.
-- Any client targeting v15 fails on prod today with "function does not exist".
--
-- v15 is self-contained — it does NOT delegate to a lower version, so this
-- migration is the only thing required to land it. v14/v13 wrappers stay put
-- for legacy callers.
