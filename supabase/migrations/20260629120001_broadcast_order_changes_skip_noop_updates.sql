-- =====================================================================
-- DB Perf Phase 6 — Lever 2 (DB-side guard): skip no-op order re-broadcasts
-- =====================================================================
-- broadcast_order_changes fires ROW-level on every orders INSERT/UPDATE/DELETE
-- (orders_broadcast_trigger_deferred has no WHEN clause). On each call it
-- rebuilds the full v2 payload + count(*) of order_items + station lookup and
-- emits realtime.send (a WAL write to realtime.messages).
--
-- Verified waste path (staging, Jun 27/29): the order_items trigger
-- propagate_rush_to_kds does `UPDATE orders SET updated_at = NOW()`, and the
-- BEFORE-UPDATE update_orders_updated_at trigger bumps updated_at on every
-- write. When a child touch changes ONLY orders.updated_at, the old function
-- still rebuilt and re-emitted an otherwise-identical payload — pure WAL
-- fan-out carrying no new information for any subscriber.
--
-- Guard: on UPDATE, if the new row is identical to the old row once updated_at
-- is excluded, emit nothing. Any change to a content-bearing column (status,
-- amounts, payment_status, timestamps other than updated_at, etc.) still
-- broadcasts exactly as before. This is intentionally minimal — it ONLY skips
-- the strict updated_at-only case, so it cannot drop a content-bearing event.
--
-- Everything else (payload shape, payment sub-fetch gating, QR emit, DELETE
-- path) is byte-for-byte the prior definition (20260610125142).
-- =====================================================================
...
begin
  ...
  -- Skip redundant re-broadcasts: an UPDATE whose only difference is
  -- updated_at carries no new information for subscribers (e.g. a child
  -- trigger bumping orders.updated_at). Emitting it is pure WAL fan-out.
  if tg_op = 'UPDATE'
...
COMMENT ON FUNCTION public.broadcast_order_changes() IS
  'Order-update broadcast trigger. v2 payload + SC snapshot columns. Skips UPDATEs whose only change is updated_at (no-op re-broadcasts from child triggers like propagate_rush_to_kds) to cut WAL fan-out; all content-bearing changes broadcast unchanged.';
