-- QR Dine-In: token-scoped guest order status realtime
--
-- Extends the existing orders broadcast trigger so qr_dine_in orders also
-- publish lightweight status updates to a session-scoped topic keyed by the
-- guest's opaque online_order_sessions.session_token.
...
comment on function public.broadcast_order_changes() is
  'Location-level order fan-out plus QR token-scoped guest status realtime for qr_dine_in orders.';
