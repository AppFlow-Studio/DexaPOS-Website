    -- ============================================
    -- Fix: enforce max_uses_per_day on manage_order_discount
    -- Changes:
    --   'apply': check daily cap (bucketed by location timezone), insert discount_usage_log
    --   'void':  mark corresponding discount_usage_log row voided
    -- ============================================
    ...
    BEGIN
        ...
        WHEN 'apply' THEN
            ...
            IF p_discount_id IS NOT NULL THEN
                ...
                -- Daily cap check (bucketed by location timezone)
                IF v_discount.max_uses_per_day IS NOT NULL THEN
                    DECLARE
                        v_daily_uses INTEGER;
                    BEGIN
                        SELECT COALESCE(timezone, 'UTC') INTO v_location_timezone
                        FROM public.locations
                        WHERE id = v_order.location_id;

                        v_local_date := (now() AT TIME ZONE v_location_timezone)::date;

                        SELECT COUNT(*) INTO v_daily_uses
                        FROM public.discount_usage_log
                        WHERE discount_id = p_discount_id
                        AND location_id = v_order.location_id
                        AND usage_date = v_local_date
                        AND voided = false;

                        IF v_daily_uses >= v_discount.max_uses_per_day THEN
                            RETURN jsonb_build_object('success', false, 'error', 'Discount daily limit reached');
                        END IF;
                    END;
                END IF;
            ...
            -- Log usage for preset discounts so daily cap can be enforced atomically
            IF v_is_preset_discount THEN
                -- Resolve local date if not already set (cap was NULL, so we skipped the block above)
                ...
                INSERT INTO public.discount_usage_log (
                    discount_id, location_id, order_id, order_item_id,
                    applied_by_staff_profiles_id, discount_amount, usage_date
                ) VALUES (
                    p_discount_id, v_order.location_id, p_order_id, NULL,
                    p_staff_id, v_calculated_amount, v_local_date
                );
            END IF;
        ...
        WHEN 'void' THEN
            ...
            -- Release the daily-cap slot for preset discounts
            IF v_voided_discount.discount_id IS NOT NULL THEN
                UPDATE public.discount_usage_log
                SET voided = true, voided_at = now()
                WHERE order_id = p_order_id
                AND discount_id = v_voided_discount.discount_id
                AND voided = false;
            END IF;
