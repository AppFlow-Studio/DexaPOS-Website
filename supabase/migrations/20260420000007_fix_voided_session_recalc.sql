-- =============================================================================
-- Fix: Allow recalculation on voided sessions
-- =============================================================================
-- When a session is voided, the same sequence_number slot should be reusable.
-- Previously, voided sessions blocked recalculation with the same error as
-- approved/exported sessions. This fix allows voided sessions to be reset to
-- draft and recalculated, while keeping the block on approved/exported sessions.
-- =============================================================================
...
BEGIN
  ...
  -- Check if a non-voided session with this sequence already exists (recalculation case)
  ...
  IF v_session_id IS NOT NULL THEN
    -- Existing non-voided session — block only approved/exported
    PERFORM 1 FROM public.tip_distribution_sessions
    WHERE id = v_session_id AND status IN ('approved', 'exported');
    IF FOUND THEN
      RAISE EXCEPTION
        'Session is in a locked state (approved/exported). '
        'Void the session first to recalculate.'
  ...
  ELSE
    -- New session (or replacing a voided one at this sequence)
    -- Delete any voided session at this slot first
    DELETE FROM public.tip_distribution_details
    WHERE session_id IN (
    ...
    DELETE FROM public.tip_distribution_sessions
    WHERE location_id = p_location_id
      AND session_date = p_session_date
      AND shift_period = p_shift_period
      AND sequence_number = v_sequence
