-- =============================================================================
-- Migration: Wire remainder distribution into calculate_tip_distribution_v2
-- Date: 2026-04-20
-- =============================================================================
-- Replaces ROUND()-based distribution in Steps 7+8 with the
-- _distribute_with_remainder and _distribute_weighted_with_remainder helpers
-- from migration 20260420000004. This eliminates penny rounding drift.
-- =============================================================================
...
BEGIN
  ...
  -- =========================================================
  SELECT data_cutoff_at, sequence_number INTO v_prev_cutoff, v_prev_seq
  ...
  ELSE
    ...
    ) VALUES (p_merchant_id, p_location_id, p_session_date, p_shift_period, v_sequence, v_data_start)
  ...
  INSERT INTO public.tip_distribution_details (
    ...
    hours_worked, gross_sales, charged_tips, cash_tips, individual_tips_earned
  )
  SELECT v_session_id, combined.staff_profile_id,
  ...
  FROM (
    ...
    shift_totals AS (
      ...
      WHERE ss.location_id = p_location_id
        AND ss.clock_in_time >= v_window_start AND ss.clock_in_time < v_window_end
      GROUP BY ss.staff_profile_id
    )
    SELECT COALESCE(sc.staff_profile_id, st.staff_profile_id) AS staff_profile_id,
      COALESCE(sc.charged_tips, 0.00) AS charged_tips, COALESCE(sc.gross_sales, 0.00) AS gross_sales,
      COALESCE(st.hours_worked, 0.00) AS hours_worked, COALESCE(st.cash_tips_declared, 0.00) AS cash_tips_declared
    FROM server_charged sc FULL OUTER JOIN shift_totals st USING (staff_profile_id)
  ) combined
  ...
  -- =========================================================
  -- STEP 2.5: CAPTURE CONFIG SNAPSHOT
  -- =========================================================
  SELECT jsonb_build_object(
    ...
    'pools', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'contributing_role_codes', tpc.contributing_role_codes, 'priority', tpc.priority,
        'policy_interval', tpc.policy_interval,
        'role_shares', (SELECT jsonb_agg(jsonb_build_object(
      ...
      WHERE tpc.location_id = p_location_id AND tpc.is_active = true
        AND tpc.effective_date <= p_session_date AND (tpc.end_date IS NULL OR tpc.end_date >= p_session_date)
    ), '[]'::jsonb),
    'tip_out_rules', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', tor.id, 'from_role_code', tor.from_role_code, 'to_role_code', tor.to_role_code,
        'tip_out_type', tor.tip_out_type, 'tip_out_value', tor.tip_out_value
      ))
      FROM public.tip_out_rules tor
      WHERE tor.location_id = p_location_id AND tor.is_active = true
        AND tor.effective_date <= p_session_date AND (tor.end_date IS NULL OR tor.end_date >= p_session_date)
    ), '[]'::jsonb)
  ) INTO v_snapshot;

  -- STEP 3: TOTAL COLLECTED
  ...
  -- STEP 4: TEMP TABLE
  CREATE TEMP TABLE temp_pool_contrib (pool_id UUID, staff_profile_id UUID, amount NUMERIC) ON COMMIT DROP;

  -- =========================================================
  -- STEP 5: POOL CONTRIBUTIONS (unchanged — per-person ROUND is fine here)
  -- =========================================================
  FOR v_pool IN
    SELECT * FROM public.tip_pool_configs
    WHERE location_id = p_location_id AND is_active = true
      AND effective_date <= p_session_date AND (end_date IS NULL OR end_date >= p_session_date)
  ...
  LOOP
    ...
    IF v_pool.tip_source = 'charged_tips' THEN
      ...
      SELECT v_pool.id, dd.staff_profile_id, ROUND(dd.charged_tips * (v_pool.source_percentage / 100.0), 2)
    ...
    ELSIF v_pool.tip_source = 'all_tips' THEN
      INSERT INTO temp_pool_contrib (pool_id, staff_profile_id, amount)
      SELECT v_pool.id, dd.staff_profile_id, ROUND(dd.individual_tips_earned * (v_pool.source_percentage / 100.0), 2)
      FROM public.tip_distribution_details dd
      WHERE dd.session_id = v_session_id AND dd.role_code = ANY(v_pool.contributing_role_codes);
    ELSIF v_pool.tip_source = 'cash_only' THEN
      INSERT INTO temp_pool_contrib (pool_id, staff_profile_id, amount)
      SELECT v_pool.id, dd.staff_profile_id, ROUND(dd.cash_tips * (v_pool.source_percentage / 100.0), 2)
      FROM public.tip_distribution_details dd
      WHERE dd.session_id = v_session_id AND dd.role_code = ANY(v_pool.contributing_role_codes);
    END IF;
  END LOOP;

  -- STEP 6: AGGREGATE CONTRIBUTIONS
  ...
  -- =========================================================
  -- STEP 7: REDISTRIBUTE EACH POOL — NOW USING REMAINDER HELPERS
  -- =========================================================
  FOR v_pool IN
    SELECT * FROM public.tip_pool_configs
    WHERE location_id = p_location_id AND is_active = true
  ...
  LOOP
    ...
    IF v_pool.distribution_method = 'percentage' THEN
      -- Each role gets its share of the pool, distributed with remainder across staff in that role
      FOR v_role_share IN
        SELECT prs.role_code, prs.share_percentage
        FROM public.tip_pool_role_shares prs
        WHERE prs.tip_pool_config_id = v_pool.id
          AND prs.is_eligible = true AND prs.share_percentage IS NOT NULL AND prs.share_percentage > 0
      LOOP
        v_role_amount := v_pool_total * (v_role_share.share_percentage / 100.0);
        PERFORM public._distribute_with_remainder(
          v_session_id, v_role_amount, 'tip_pool_received',
          format('role_code = %L', v_role_share.role_code)
        );
      END LOOP;

    ELSIF v_pool.distribution_method = 'equal_split' THEN
      -- Equal split across all eligible staff
      PERFORM public._distribute_with_remainder(
        v_session_id, v_pool_total, 'tip_pool_received',
        format('EXISTS (SELECT 1 FROM public.tip_pool_role_shares prs WHERE prs.tip_pool_config_id = %L AND prs.role_code = tip_distribution_details.role_code AND prs.is_eligible = true)', v_pool.id)
      );

    ELSIF v_pool.distribution_method = 'hours_weighted' THEN
      -- Weighted by hours_worked
      PERFORM public._distribute_weighted_with_remainder(
        v_session_id, v_pool_total, 'tip_pool_received',
        format('EXISTS (SELECT 1 FROM public.tip_pool_role_shares prs WHERE prs.tip_pool_config_id = %L AND prs.role_code = tip_distribution_details.role_code AND prs.is_eligible = true)', v_pool.id),
        'dd.hours_worked'
      );
    ...
    ELSIF v_pool.distribution_method = 'points' THEN
      -- Weighted by points_per_hour * hours_worked
      PERFORM public._distribute_weighted_with_remainder(
        v_session_id, v_pool_total, 'tip_pool_received',
        format('EXISTS (SELECT 1 FROM public.tip_pool_role_shares prs WHERE prs.tip_pool_config_id = %L AND prs.role_code = tip_distribution_details.role_code AND prs.is_eligible = true)', v_pool.id),
        format('(SELECT prs2.points_per_hour FROM public.tip_pool_role_shares prs2 WHERE prs2.tip_pool_config_id = %L AND prs2.role_code = dd.role_code AND prs2.is_eligible = true LIMIT 1) * dd.hours_worked', v_pool.id)
      );
    END IF;
  END LOOP;

  -- =========================================================
  -- STEP 8: TIP-OUT RULES — NOW USING REMAINDER HELPERS FOR RECEIVER SPLIT
  -- =========================================================
  FOR v_rule IN
  ...
  LOOP
    ...
    IF v_rule.tip_out_type = 'percentage_of_sales' THEN
      ...
      SELECT COALESCE(SUM(ROUND(dd.gross_sales * (v_rule.tip_out_value / 100.0), 2)), 0) INTO v_tipout_total
      FROM public.tip_distribution_details dd WHERE dd.session_id = v_session_id AND dd.role_code = v_rule.from_role_code;
      -- Distribute to receivers with remainder method
      IF v_receiver_count > 0 THEN
        PERFORM public._distribute_with_remainder(
          v_session_id, v_tipout_total, 'tip_out_received',
          format('role_code = %L', v_rule.to_role_code)
        );
      END IF;
    ...
    ELSIF v_rule.tip_out_type = 'percentage_of_tips' THEN
      ...
      WHERE dd.session_id = v_session_id AND dd.role_code = v_rule.from_role_code;
      SELECT COALESCE(SUM(ROUND(dd.individual_tips_earned * (v_rule.tip_out_value / 100.0), 2)), 0) INTO v_tipout_total
      FROM public.tip_distribution_details dd WHERE dd.session_id = v_session_id AND dd.role_code = v_rule.from_role_code;
      IF v_receiver_count > 0 THEN
        PERFORM public._distribute_with_remainder(
          v_session_id, v_tipout_total, 'tip_out_received',
    ...
    ELSIF v_rule.tip_out_type = 'flat_amount' THEN
      ...
      FROM public.tip_distribution_details WHERE session_id = v_session_id AND role_code = v_rule.from_role_code;
  ...
  WITH capped AS (
    ...
    FROM public.tip_distribution_details WHERE session_id = v_session_id AND tip_out_given > 0
  )
  UPDATE public.tip_distribution_details dd
  ...
  UPDATE public.tip_distribution_sessions
  SET status = 'calculated', total_tips_collected = v_total_collected,
      ...
      config_snapshot = v_snapshot, calculated_at = now(),
      calculated_by = p_calculated_by, data_cutoff_at = v_window_end
  WHERE id = v_session_id;
  ...
  RETURN json_build_object(
    ...
    'details', (
      SELECT json_agg(json_build_object(
        ...
        'role_code', role_code, 'gross_sales', gross_sales, 'charged_tips', charged_tips,
        'cash_tips', cash_tips, 'individual_tips_earned', individual_tips_earned,
