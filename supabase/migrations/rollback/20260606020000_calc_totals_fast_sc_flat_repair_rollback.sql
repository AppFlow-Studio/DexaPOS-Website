-- Rollback for 20260606020000_calc_totals_fast_sc_flat_repair.sql.
--
-- Re-installs the pre-repair (pro-rated-from-items) body so the repair can be
-- reverted without snapshotting prod first. This body was captured from prod
-- (project hifouuofcaytijrkbvcy) before the repair migration was applied.
...
SELECT COALESCE(r.is_taxable, false) INTO v_sc_is_taxable
FROM public.service_charge_rules r WHERE r.id = v_order.service_charge_rule_id;
...
SELECT
    COALESCE(SUM(ROUND(subtotal * LEAST(quantity, quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0))::NUMERIC / NULLIF(quantity, 0), 2) + ROUND(tax_amount * LEAST(quantity, quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0))::NUMERIC / NULLIF(quantity, 0), 2)), 0),
    COALESCE(SUM(ROUND(cash_subtotal * LEAST(quantity, quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0))::NUMERIC / NULLIF(quantity, 0), 2) + ROUND(cash_tax_amount * LEAST(quantity, quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0))::NUMERIC / NULLIF(quantity, 0), 2)), 0)
...
SELECT
    COALESCE(SUM(COALESCE(original_amount, amount) - COALESCE(refunded_amount, 0) * COALESCE(original_amount, amount) / NULLIF(amount, 0)), 0),
...
FROM public.order_payments
WHERE order_id = p_order_id AND (status = 'void' OR is_voided = true);
