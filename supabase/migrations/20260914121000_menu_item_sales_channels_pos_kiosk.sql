-- ============================================================================
-- Sales Channels (POS / Kiosk): make the dashboard toggle actually reach the
-- devices.
--
-- THE BUG. The item form's "Sales Channels" checkboxes have always written
-- menu_items.available_channels (L1) and location_item_overrides
-- .available_channels (L2), but the only RPC that ever read those columns was
-- get_items_for_location_library — a dashboard-only query behind the Items
-- table. get_menu_with_categories, which builds every menu item the POS and the
-- kiosk render (via get_pos_bootstrap_v1/v2), never emitted the field, so the
-- checkbox changed nothing on any device. Unticking POS on an item still sold
-- it on POS.
--
-- This migration is the server half of the fix: resolve the cascade once,
-- server-side, and hand the devices one `effective_available_channels` array.
-- The client half (filtering) lands in the Dexa-POS repo.
--
-- ----------------------------------------------------------------------------
-- THE KIOSK BACKFILL, and why it is not optional.
--
-- menu_items.available_channels was created with DEFAULT '["pos","online"]' —
-- **kiosk is not in it**. The dashboard's own form defaults match (three
-- separate spots in NewEditItemFormSheet plus DEFAULT_ITEM_CHANNELS), so
-- essentially every item in the database says "not sold on kiosk" today. That
-- was harmless while nothing read the column. The moment the kiosk starts
-- honouring it, every one of those items disappears from the kiosk — the whole
-- menu goes blank.
--
-- So: add 'kiosk' to every array that is missing it, at both levels. There is
-- no way to tell "column default" from "someone deliberately ticked exactly
-- pos+online", because the two are byte-identical. The tiebreaker is that the
-- toggle has never had any effect on any surface, so no merchant can hold a
-- working expectation that unticking Kiosk hid anything. Backfilling to visible
-- reproduces exactly today's observable behaviour on deploy day, and the toggle
-- becomes live from the next edit onward.
--
-- 'pos' gets NO backfill, deliberately. It is already in the column default, so
-- an item missing it was unticked by hand — real intent, and honouring it is
-- the point of the ticket.
--
-- ROLLBACK: the backfill is one-way (a re-ticked kiosk box is indistinguishable
-- from one that was always ticked). To back out the behaviour, revert the
-- client filter — the extra JSON field is inert on its own.
-- ============================================================================

-- 1. Backfill L1. jsonb_typeof guards the handful of rows that could hold a
--    non-array (NULL stays NULL and is treated as "all channels" downstream).
UPDATE public.menu_items
   SET available_channels = available_channels || '["kiosk"]'::jsonb
 WHERE jsonb_typeof(available_channels) = 'array'
   AND NOT (available_channels ? 'kiosk');

-- 2. Backfill L2. NULL here means "no override" and must stay NULL — only rows
--    that already carry an explicit array are touched. These inherited the same
--    pos+online artefact from the form default whenever a location override was
--    saved, so they need the same correction.
UPDATE public.location_item_overrides
   SET available_channels = available_channels || '["kiosk"]'::jsonb
 WHERE jsonb_typeof(available_channels) = 'array'
   AND NOT (available_channels ? 'kiosk');

-- 3. Stop minting the artefact. Without this, every item created after the
--    backfill is invisible on the kiosk again.
ALTER TABLE public.menu_items
  ALTER COLUMN available_channels SET DEFAULT '["pos", "online", "kiosk"]'::jsonb;

COMMENT ON COLUMN public.menu_items.available_channels IS
  'Sales channels this item is sold on: any of pos / online / kiosk. NULL means all channels (clients fail open).';
COMMENT ON COLUMN public.location_item_overrides.available_channels IS
  'Per-location override of menu_items.available_channels. NULL means no override — inherit L1.';

-- ============================================================================
-- 4. Re-declare get_menu_with_categories verbatim from
--    20260728140000_menu_rpc_storefront_badges.sql with ONE field added to the
--    menu_item JSON: effective_available_channels. `lio` (the L2 override) is
--    already joined for pricing, so this adds no joins and no cost. Pricing,
--    availability, 86ing and badge logic are untouched.
-- ============================================================================
                        ...
                        SELECT COALESCE(json_agg(
                            json_build_object(
                                ...
                                'menu_item', json_build_object(
                                    ...
                                    -- Storefront badges (per-location, L2 only).
                                    -- Emitted at the top level so consumers do not
                                    -- have to reach into location_override.
                                    'is_popular', COALESCE(lio.is_popular, false),
                                    'is_new', COALESCE(lio.is_new, false),

                                    -- ============================================
                                    -- SALES CHANNELS: L2 override > L1 base.
                                    -- NULL at both levels means the item predates
                                    -- the column; clients fail open on a missing
                                    -- or non-array value, and so does this.
                                    -- ============================================
