-- Repair: restore the HQ board-mirror snapshot capture inside
-- bulk_update_order_item_status_v2.
--
-- 20260827160000_fix_kds_refire_preserves_fire_time.sql was applied to STAGING
-- out-of-band from an earlier revision that carried only the re-fire COALESCE
-- fix and NOT the ready/served `capture_kds_board_snapshots_for_items` call that
-- 20260827150000_hq_kds_board_mirror.sql adds. Because both redeclare the same
-- function and 160000 sorts later, that stale copy rolled the snapshot capture
-- back on staging (`has_snapshot_capture = false`). The version is already
-- recorded, so a normal `db push` will not re-apply the corrected 160000 body.
--
-- This migration re-declares the function with the canonical merged body:
--   base (20260814130000) + board-mirror snapshot capture + 'sent' COALESCE fix.
-- It is idempotent: wherever 160000 was applied from the corrected file (e.g.
-- production via the promotion PR), this is a no-op replace with an identical
-- body. Behaviour-neutral vs the corrected 160000.

-- Four-argument overload stays dropped (matches the traceability migration).
...
BEGIN
  ...
  -- ==== Board-mirror snapshot capture (HQ KDS mirror, P1) ==================
  ...
  -- From 20260827150000_hq_kds_board_mirror.sql.
...
COMMENT ON FUNCTION public.bulk_update_order_item_status_v2(
  ...
  'Bulk-updates order item/KDS status with idempotency and optimistic concurrency. Re-firing a sent item preserves fire_time (original KDS ticket) via COALESCE. Ready/served capture kds_board_snapshots for the HQ board mirror.';
