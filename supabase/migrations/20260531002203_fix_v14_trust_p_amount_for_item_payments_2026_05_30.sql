...
BEGIN
    ...
    IF v_order.payment_status = 'paid' AND COALESCE(v_order.amount_due, 0) <= 0 THEN RAISE EXCEPTION 'Order is already fully paid'; END IF;
    ...
    IF v_is_split_payment THEN
        ...
        INTO v_portions_paid, v_paid_portion_indexes FROM public.order_payments
    ...
    IF v_is_item_payment THEN
        ...
        IF p_amount IS NOT NULL AND p_amount > 0 THEN
            v_payment_total := LEAST(p_amount, CASE WHEN v_use_cash_pricing THEN v_pre_unpaid_cash_total ELSE v_pre_unpaid_card_total END);
        ...
        SELECT COALESCE(jsonb_agg(jsonb_build_object('order_item_id', pc.id, 'item_name', pc.item_name, 'quantity_paid', pc.qty_paying, 'original_quantity', pc.original_qty, 'unit_price', CASE WHEN v_is_cash THEN pc.cash_price ELSE pc.unit_price END, 'tax_rate', COALESCE(pc.tax_rate, 0), 'prorated_discount', CASE WHEN v_is_cash THEN pc.prorated_cash_discount ELSE pc.prorated_discount END, 'subtotal', CASE WHEN v_is_cash THEN (pc.qty_paying * pc.cash_price) - pc.prorated_cash_discount ELSE (pc.qty_paying * pc.unit_price) - pc.prorated_discount END)), '[]'::jsonb)
    ...
    SELECT COALESCE(SUM(COALESCE(service_charge, 0)), 0) INTO v_prior_sc_snapshot
    FROM public.order_payments WHERE order_id = p_order_id AND status IN ('captured', 'partially_refunded', 'refunded') AND is_voided = false;

    v_remaining_sc := GREATEST(COALESCE(v_order.service_charge, 0) - v_prior_sc_snapshot, 0);

    IF v_use_cash_pricing AND COALESCE(v_order.cash_total, 0) > 0 THEN v_payment_card_equiv := ROUND(v_payment_total * v_order.card_total / v_order.cash_total, 2);
    ELSE v_payment_card_equiv := v_payment_total; END IF;

    v_remaining_after_payment := GREATEST(v_payment_based_due - v_payment_card_equiv, 0);

    IF v_is_item_payment THEN
        SELECT COUNT(*) INTO v_unpaid_items_after_payment
        FROM public.order_items oi WHERE oi.order_id = p_order_id AND oi.is_voided = false AND (oi.quantity - COALESCE(oi.paid_quantity, 0) + COALESCE(oi.refunded_quantity, 0)) > 0;
        v_closes_with_items := (v_unpaid_items_after_payment = 0);
    END IF;

    IF v_remaining_sc <= 0 THEN v_service_charge_share := 0;
    ELSIF (v_is_split_payment AND v_is_last_portion) OR v_is_full_remaining OR v_remaining_after_payment <= 0.05 OR v_closes_with_items THEN v_service_charge_share := v_remaining_sc;
    ELSIF v_use_cash_pricing AND COALESCE(v_order.cash_total, 0) > 0 THEN v_service_charge_share := LEAST(v_remaining_sc, ROUND(v_payment_total * v_order.service_charge / v_order.cash_total, 2));
    ELSIF COALESCE(v_order.card_total, 0) > 0 THEN v_service_charge_share := LEAST(v_remaining_sc, ROUND(v_payment_total * v_order.service_charge / v_order.card_total, 2));
    ELSE v_service_charge_share := 0; END IF;

    IF v_is_item_payment AND v_service_charge_share > 0 THEN
        IF p_amount IS NOT NULL AND p_amount > 0 THEN
            v_service_charge_share := LEAST(v_remaining_sc, GREATEST(v_payment_total - (v_items_subtotal + v_items_tax), 0));
        ELSE
    ...
    PERFORM log_payment_event(
        ...
        p_response_message := CASE WHEN v_is_cash THEN 'Cash payment captured' ELSE COALESCE(p_terminal_response->'dejavoo_transaction'->>'resultMessage', p_terminal_response->'castles_transaction'->>'statusMessage', 'Card payment captured successfully') END,
    ...
    SELECT COALESCE(SUM(COALESCE(original_amount, amount) - COALESCE(refunded_amount, 0) * COALESCE(original_amount, amount) / NULLIF(amount, 0)), 0)
    INTO v_effective_paid FROM public.order_payments WHERE order_id = p_order_id AND status IN ('captured', 'partially_refunded', 'refunded') AND is_voided = false;

    SELECT COALESCE(SUM(
        CASE WHEN is_cash_priced THEN amount - COALESCE(refunded_amount, 0)
        ELSE CASE WHEN COALESCE(v_order.card_total, 0) > 0 THEN ROUND((amount - COALESCE(refunded_amount, 0)) * v_order.cash_total / v_order.card_total, 2) ELSE amount - COALESCE(refunded_amount, 0) END
        END), 0)
    INTO v_effective_cash_paid FROM public.order_payments WHERE order_id = p_order_id AND status IN ('captured', 'partially_refunded', 'refunded') AND is_voided = false;
    ...
    IF v_is_split_payment THEN
        IF v_order_fully_paid AND v_unpaid_items_count > 0 THEN
            UPDATE public.order_items SET paid_quantity = quantity, refunded_quantity = 0, refunded_amount = 0, price_paid = COALESCE(price_paid, unit_price), updated_at = now()
            WHERE order_id = p_order_id AND is_voided = false AND (quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0)) > 0;
    ...
    ELSE
        ...
        IF v_order_fully_paid AND v_unpaid_items_count > 0 THEN
            UPDATE public.order_items SET paid_quantity = quantity, refunded_quantity = 0, refunded_amount = 0, price_paid = COALESCE(price_paid, CASE WHEN v_use_cash_pricing THEN cash_price ELSE unit_price END), updated_at = now()
            WHERE order_id = p_order_id AND is_voided = false AND (quantity - COALESCE(paid_quantity, 0) + COALESCE(refunded_quantity, 0)) > 0;
            v_unpaid_items_count := 0; v_unpaid_card_total := 0; v_unpaid_cash_total := 0;
        END IF;
    END IF;
...
GRANT EXECUTE ON FUNCTION public.process_payment_v14(uuid, text, numeric, numeric, numeric, jsonb, uuid, jsonb, integer, integer, boolean, uuid, uuid, uuid) TO authenticated;;
