-- ============================================================================
-- prepare_castles_settlement: stamp settlement_batches.origin (pos_auto/pos_manual).
-- ----------------------------------------------------------------------------
-- Rebased on the LIVE definition (verbatim) with ONE addition: the batch is
-- stamped with origin='pos_auto' when the POS scheduler initiates it
-- (p_initiated_by IN 'pos_auto'|'scheduler'|'auto'), else 'pos_manual'.
-- Backward-compatible — existing callers pass a user id => 'pos_manual'. All
-- settlement math / guards / host-keyed vs fallback logic is unchanged.
-- Does NOT touch finalize (origin is set once, at batch creation in prepare).
-- ============================================================================
...
BEGIN
    ...
    IF v_is_host_keyed THEN
        ...
        UPDATE public.settlement_batches
        SET status='pending', origin=v_origin, transaction_count=v_payment_count, gross_amount=v_gross,
    ...
    ELSE
        ...
        INSERT INTO public.settlement_batches (
            ...
            status, origin, castles_pos_txn_id, opened_at, created_at, updated_at
        ) VALUES (
            ...
            'pending', v_origin, v_pos_txn_id, NOW(), NOW(), NOW()
