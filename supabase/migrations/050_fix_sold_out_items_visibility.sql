-- Migration 050: Fix sold-out item visibility in storefront
--
-- Previously, get_menu_with_categories filtered out items with
-- ci.is_available = false in the WHERE clause, making them invisible
-- on the storefront. The storefront now shows sold-out items grayed
-- out with a "Sold Out" badge. The effective_availability field
-- already computes the correct false value — we just need to stop
-- pre-filtering those items out.
