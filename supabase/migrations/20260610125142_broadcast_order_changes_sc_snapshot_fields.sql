-- =====================================================================
-- Migration: broadcast_order_changes — include SC snapshot columns
-- =====================================================================
-- The trigger builds the broadcast payload by jsonb_build_object'ing each
-- column by name. It was missing the service-charge snapshot fields:
...
--
-- Client transformer (utils/orderTransformers.ts) maps missing fields to
-- defaults — service_charge_is_manual ?? false — so every broadcast from a
-- discount/item-edit update wiped a manager's manual override flag client-
-- side. Local calculator then hit the ruleEligible branch and replaced the
-- manual SC dollar amount with rule × subtotal (the user-reported "$5 → $2.84
-- (18%)" after applying a discount). DB stayed correct; only the client view
-- drifted until the next full sync.
--
-- This migration extends the order_data payload with the SC snapshot
-- columns. All other broadcast behavior is unchanged.
-- =====================================================================
