-- Migration: add_open_item_v3 — add per-item "TO GO" flag at creation
-- Appends p_is_to_go (default false) and writes order_items.is_to_go on insert.
-- Mirrors the per-item is_to_go support already present on regular items
-- (column + toggle_to_go_order_items + get_kds_tickets_v2). Open/custom items
-- are order_items rows with is_open_item = true, so the toggle/KDS already
-- cover them post-creation; this closes the creation-path gap.
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.add_open_item_v3(uuid, text, numeric, integer, text, boolean, integer, uuid, uuid, boolean) TO authenticated;
