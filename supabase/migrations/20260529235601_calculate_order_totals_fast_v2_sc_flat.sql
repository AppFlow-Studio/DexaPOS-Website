...
v_sc_already_collected numeric;
v_sc_residual numeric;
...
v_sc_residual := LEAST(
    GREATEST(v_service_charge - v_sc_already_collected, 0),
    v_custom_refund_balance
);
v_refund_balance := GREATEST(v_custom_refund_balance - v_sc_residual, 0);
...
ELSE
    ...
    v_unpaid_cash_total := v_unpaid_cash_total + v_sc_residual +
      CASE WHEN v_card_total_calc > 0
      THEN ROUND(v_refund_balance * v_cash_total_calc / v_card_total_calc, 2)
      ELSE v_refund_balance END;
END IF;
...
COMMENT ON FUNCTION public.calculate_order_totals_fast(uuid) IS
  'Wave D follow-up (v2 in place). v_custom_refund_balance is split into SC residual (flat $ across cash/card pricing) and refund balance (scaled by cash/card ratio). Previously the entire balance was scaled, causing cash_amount_due to under-collect SC by (1 - cash_total/card_total) * SC on dual-priced orders (S6-0016 staging 2026-05-29: $3.25 instead of $3.71).';;
