...
--
-- Race: close_cash_drawer_session and record_cash_operation both
--   (a) SUM cash_drawer_operations to compute expected_cash, then
--   (b) UPDATE cash_drawer_sessions.expected_cash
-- without holding a row lock. Two concurrent calls interleave their
-- SELECT/UPDATE windows and the last UPDATE wins, leaving expected_cash
-- out of sync with SUM(operations).
--
-- Fix: SELECT ... FROM public.cash_drawer_sessions WHERE id = p_session_id
--      FOR UPDATE
-- at the top of both RPCs. Postgres row-level lock serializes them; the
-- second waits until the first commits and re-evaluates session.status.
--
-- Style: matches the post-Apr-30 hardening pattern from
-- 20260430154957_qualify_cash_drawer_rpc_bodies.sql — empty search_path
-- with every public.* reference qualified.
--
-- Also B3: emit an audit_logs entry on close documenting that
-- expected_cash was computed from SUM(cash_drawer_operations) under lock.
...
CREATE OR REPLACE FUNCTION public.close_cash_drawer_session(
  ...
  p_closing_count_details jsonb DEFAULT NULL::jsonb,
  p_variance_notes text DEFAULT NULL::text,
...
DECLARE
  ...
  v_op_count INT;
BEGIN
  -- Lock the session row. Any concurrent record_cash_operation blocks until
  -- this close commits. After commit, that RPC finds status='closed' and
  -- rejects — which is the correct behavior.
  SELECT * INTO v_session
  FROM public.cash_drawer_sessions
  WHERE id = p_session_id AND status = 'open'
  FOR UPDATE;
  ...
  -- Compute expected_cash from SUM(cash_drawer_operations) under the lock.
  -- No in-flight write can sneak in between this SELECT and our UPDATE.
  SELECT
  ...
  -- B3: audit log — document expected_cash was derived from
  -- SUM(cash_drawer_operations) under FOR UPDATE row lock.
  INSERT INTO public.audit_logs (
    action,
    action_category,
    severity,
