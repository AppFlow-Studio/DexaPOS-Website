...
-- Fix: calculate_tip_distribution_v2 shift_totals CTE excludes staff who
-- clocked in before the session window but are still active. These staff
-- should appear in the distribution to receive pool shares.
--
-- Change: shift_totals now includes shifts that started in the business day
-- AND either started after the window start OR are still active (no clock_out).
...
BEGIN
  ...
  -- STEP 1: SESSION SEQUENCE + TIME WINDOW
  ...
  v_window_end   := CASE
    WHEN p_session_date < CURRENT_DATE THEN v_day_end
    ELSE now()
  END;

  -- STEP 2.5: POPULATE FROM SOURCE TABLES
  -- Orders are strictly windowed (only new orders since last cutoff).
  -- Shifts include those started after cutoff OR still active (no clock_out),
  -- so staff who clocked in before Session #1 but are still working appear
  -- in Session #2 to receive pool distributions.
  ...
  FROM (
    ...
    shift_totals AS (
      ...
      WHERE ss.location_id = p_location_id
        ...
        -- Include shifts started after cutoff OR still active (for multi-session)
