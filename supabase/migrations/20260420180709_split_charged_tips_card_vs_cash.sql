...
-- ============================================================================
-- Migration: Split charged_tips into card-only vs cash payment tips
--
-- Problem: charged_tips in employee_daily_tips lumps ALL order_payments.tip_amount
-- together. Pools configured with tip_source='charged_tips' (card only) or
-- 'cash_only' get wrong data because cash payment tips are in charged_tips
-- and missing from cash_tips.
--
-- Fix: Add cash_payment_tips column, update rebuild + calculate RPCs to split
-- by payment_method, update the generated total_tips column.
-- ============================================================================

-- 1. Add cash_payment_tips column to employee_daily_tips
ALTER TABLE public.employee_daily_tips
  ADD COLUMN IF NOT EXISTS cash_payment_tips NUMERIC(12,2) NOT NULL DEFAULT 0;

-- 2. Drop and recreate the generated total_tips column to include cash_payment_tips
ALTER TABLE public.employee_daily_tips DROP COLUMN IF EXISTS total_tips;
ALTER TABLE public.employee_daily_tips
  ADD COLUMN total_tips NUMERIC GENERATED ALWAYS AS (
    (((((cash_tips_declared + cash_payment_tips + charged_tips) + tip_pool_received) - tip_pool_contributed) + tip_out_received) - tip_out_given)
  ) STORED;

-- 3. Update rebuild_employee_daily_tips to split card vs cash payment tips
CREATE OR REPLACE FUNCTION public.rebuild_employee_daily_tips(p_location_id uuid, p_shift_date date)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  combined AS (
    SELECT
      ...
      COALESCE(st2.card_tips, 0.00)            AS charged_tips,
  ...
  INSERT INTO public.employee_daily_tips (
    ...
    charged_tips, cash_payment_tips, gross_sales, hours_worked, cash_tips_declared
  )
  SELECT c.staff_profile_id, v_merchant_id, p_location_id, p_shift_date,
    c.charged_tips, c.cash_payment_tips, c.gross_sales, c.hours_worked, c.cash_tips_declared
...
-- 4. Update calculate_tip_distribution_v2 to split card vs cash payment tips
-- and map cash_payment_tips into cash_tips (so cash_only pools get them)
...
BEGIN
  ...
  -- STEP 1.5: REFRESH DAILY TIP ROLLUPS
  ...
  JOIN public.location_members lm
    ON lm.staff_profile_id = combined.staff_profile_id AND lm.location_id = p_location_id
  JOIN public.staff_profiles sp ON sp.id = combined.staff_profile_id
  WHERE combined.staff_profile_id IS NOT NULL;

  -- STEP 2.5: CONFIG SNAPSHOT
  SELECT jsonb_build_object(
    'snapshot_taken_at', now(),
    'pools', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', tpc.id, 'name', tpc.name,
...
-- 5. Update preview_tip_distribution (it wraps calculate, so it picks up the fix automatically)
-- No change needed — it calls calculate_tip_distribution_v2 internally.

-- 6. Backfill: rebuild all existing employee_daily_tips to split card/cash
-- This is a one-time operation. For each distinct (location_id, shift_date) in
-- employee_daily_tips, re-run rebuild to split the tips correctly.
-- Note: We can't call rebuild_employee_daily_tips here because it requires auth context.
-- Instead, directly update from order_payments data.
UPDATE public.employee_daily_tips edt
SET cash_payment_tips = COALESCE((
  SELECT SUM(op.tip_amount)::NUMERIC(12,2)
  FROM public.order_payments op
  JOIN public.orders o ON o.id = op.order_id
  WHERE (o.assigned_server_id = edt.staff_profile_id OR o.created_by_staff_id = edt.staff_profile_id)
    AND o.location_id = edt.location_id
    AND op.payment_method = 'cash'
    AND op.status = 'captured'
    AND o.created_at::date = edt.shift_date
), 0),
charged_tips = COALESCE((
  SELECT SUM(op.tip_amount)::NUMERIC(12,2)
  FROM public.order_payments op
  JOIN public.orders o ON o.id = op.order_id
  WHERE (o.assigned_server_id = edt.staff_profile_id OR o.created_by_staff_id = edt.staff_profile_id)
