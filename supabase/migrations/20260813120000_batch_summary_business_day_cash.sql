-- =====================================================================
-- Batch summary — business-day cash (all-tender) block
-- =====================================================================
-- Why: get_batch_summary_v1 scopes to order_payments.settlement_batch_id.
-- Cash payments are written with terminal_type='cash_drawer' and are
-- deliberately skipped by _lazy_settlement_batch_link, so they never carry a
-- settlement_batch_id. The batch summary therefore always printed Cash
-- $0.00 even on days with hundreds of dollars of cash sales.
--
-- A card settlement batch is an acquirer concept — cash does not belong to
-- it. Folding cash into the batch gross/net would misstate the bank deposit
-- (cash never moves through the processor). Instead we add a separate,
-- informational BUSINESS DAY (all tenders) block computed over the batch's
-- business-day window for the batch's LOCATION. The card batch
-- header/sales/refunds/net figures below are byte-for-byte unchanged from
-- wave H.7.
--
-- The predicate mirrors get_business_day_activity_summary_v1 exactly (same
-- get_business_day_bounds window, same in_sales discriminator, same
-- p.location_id filter, same cash/card/gift/house payment_method matching)
-- so the printed cash figure reconciles with "Print Today's Summary" for the
-- same business date.
--
-- Multi-terminal note: if a location batches out more than one terminal in a
-- day, each batch summary repeats the SAME business-day cash figure (it is a
-- day total, not a per-batch total). That is intentional and safe because
-- the block is informational and is NOT summed into the card net deposit.
-- The section title makes the all-tender / whole-day scope explicit.
--
-- Edge: for legacy batches with a NULL business_date, get_business_day_bounds
-- falls back to "today", so a reprint of a very old batch shows today's cash.
-- Acceptable — new batches always stamp business_date.
-- =====================================================================
...
DECLARE
    ...
    v_tip numeric;
    v_net numeric;
    -- Business-day (all-tender) block — cash has no settlement batch.
    v_bd_bounds RECORD;
    v_bd_cash numeric;
    v_bd_cash_cnt int;
    v_bd_card numeric;
...
BEGIN
    ...
    -- ---------------------------------------------------------------
    -- Business-day all-tender totals for this batch's location + day.
    -- Independent of settlement_batch_id (cash never carries one). Kept
    -- OUT of the card figures above. Same predicate as
    -- get_business_day_activity_summary_v1 so cash reconciles with the
    -- closing report.
    -- ---------------------------------------------------------------
    SELECT * INTO v_bd_bounds
        FROM get_business_day_bounds(v_batch.location_id, v_batch.business_date);

    SELECT
        COALESCE(SUM(amount) FILTER (WHERE in_sales AND payment_method::text = 'cash'), 0),
        COUNT(*)             FILTER (WHERE in_sales AND payment_method::text = 'cash'),
        COALESCE(SUM(amount) FILTER (WHERE in_sales AND payment_method IN ('card','card_spinapi','card_dvpaylite','card_manual','card_online')), 0),
        COALESCE(SUM(amount) FILTER (WHERE in_sales AND payment_method::text = 'gift_card'), 0),
        COALESCE(SUM(amount) FILTER (WHERE in_sales AND payment_method::text = 'house_account'), 0),
        COALESCE(SUM(amount) FILTER (WHERE in_sales), 0)
    INTO v_bd_cash, v_bd_cash_cnt, v_bd_card, v_bd_gift, v_bd_house, v_bd_gross
    FROM (
        ...
        FROM order_payments
        WHERE location_id = v_batch.location_id
          AND captured_at >= v_bd_bounds.start_ts
          AND captured_at <  v_bd_bounds.end_ts
    ) q;
    ...
    RETURN jsonb_build_object(
        ...
        -- All-tender totals for the batch's business day. Cash lives ONLY
        -- here (no settlement batch). Not summed into net_deposit above.
        'business_day', jsonb_build_object(
            'business_date', COALESCE(v_batch.business_date, (v_bd_bounds.start_ts AT TIME ZONE 'UTC')::date),
            'cash_total',    v_bd_cash,
            'cash_count',    v_bd_cash_cnt,
            'card_total',    v_bd_card,
            'gift_total',    v_bd_gift,
            'house_total',   v_bd_house,
            'gross',         v_bd_gross
        ),
...
COMMENT ON FUNCTION public.get_batch_summary_v1(uuid) IS
  'Settled-batch summary. Card figures (sales/refunds/net) scope to settlement_batch_id. Adds a business_day block (all-tender totals over get_business_day_bounds for the batch location) so cash — which has no settlement_batch_id — appears on the printed receipt without inflating the card net deposit.';
