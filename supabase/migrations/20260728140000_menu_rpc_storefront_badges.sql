-- ============================================================================
-- Storefront badges: expose is_popular / is_new from get_menu_with_categories.
--
-- The Popular badge is a per-location flag on location_item_overrides, set from
-- the dashboard item page. The storefront maps it to StorefrontItem.is_popular
-- and uses it to build the "Popular" row on the market template.
--
-- Before this migration the RPC never returned it at the top level (an older
-- revision emitted it only nested inside a `location_override` object, and the
-- current revision dropped that object entirely), and is_new was not returned
-- at all. So every storefront item came through with is_popular = false and the
-- Popular section could never render, no matter what the merchant flagged.
--
-- This re-declares get_menu_with_categories() verbatim from
-- 20260720180000_category_86_snooze.sql with two fields added to the menu_item
-- JSON. Pricing, availability and 86ing logic are untouched.
-- ============================================================================
