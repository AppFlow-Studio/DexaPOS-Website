CREATE OR REPLACE FUNCTION public.apply_refund_to_payment_v2(
  ...
  p_return_rrn text DEFAULT NULL, p_return_auth_code text DEFAULT NULL,
  p_return_reference_id text DEFAULT NULL, p_return_number text DEFAULT NULL,
  p_return_reason text DEFAULT NULL, p_initiated_by uuid DEFAULT NULL,
  p_restore_paid_quantity boolean DEFAULT false, p_idempotency_key UUID DEFAULT NULL
)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  v_cached JSONB; v_payment record; v_new_refunded numeric; v_new_status payment_status; v_ci record;
BEGIN
  ...
  ELSE v_new_status := 'partially_refunded'::payment_status;
  END IF;
