-- ============================================================================
-- FIX: Restore effective_availability on get_menu_with_categories items
-- ============================================================================
-- Regression context:
--   effective_availability was present on each menu_item up through
--   20260427005000_pricing_cascade_fix.sql, then silently dropped when the
--   function was rewritten for modifier display-ordering
--   (20260604183502 / 20260610121342). Every consumer of this RPC lost the
--   field:
--     * OrderOut menu sync (lib/orderout/transform-menu.ts) skips any item
--       where effective_availability is falsy -> undefined nuked the whole
--       menu, syncing 0 items even though categories/items exist.
--     * Public storefront + dashboard/manage menu views fall back to
--       `?? true`, so genuinely unavailable (sold-out) items render as
--       available everywhere.
--
-- This migration re-applies the CURRENT live definition of
-- get_menu_with_categories verbatim (preserving the modifier display_order
-- + location_item_modifier_groups UNION behavior) and adds
-- effective_availability back to each menu_item using the full 6-level
-- availability cascade (AND logic across L1..L5, incl. the global
-- menu-category row ci_menu).
-- ============================================================================
