-- =====================================================================
-- Migration: broadcast_order_changes — include platform_order_number
-- =====================================================================
-- Online-order cards (Kanban / right-edge drawer) show the marketplace's own
-- order number for delivery-platform orders. platform_order_number is set by
-- process_online_order (website 20260702120001) but the broadcast payload
-- omitted it, so cards created from a live broadcast fell back to the Dexa
-- display_number until the next full fetch. Adds the column to the payload
-- (metadata->>'provider_order_id' fallback for rows created before the column
-- was populated). All other broadcast behavior is unchanged from
-- 20260610125142_broadcast_order_changes_sc_snapshot_fields.
-- =====================================================================
...
COMMENT ON FUNCTION public.broadcast_order_changes() IS
  'Order-update broadcast trigger. v2 payload + SC snapshot columns + platform_order_number. Without the SC snapshots, the client transformer mapped missing service_charge_is_manual ?? false on every broadcast and clobbered manager overrides client-side after any orders-row UPDATE.';
