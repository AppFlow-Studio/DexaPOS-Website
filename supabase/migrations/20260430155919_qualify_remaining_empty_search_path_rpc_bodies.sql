-- Fix: 42P01 / 42883 from RPCs with `SET search_path = ''` whose bodies
-- still reference public.* tables/types/functions without qualification.
--
-- Companion to:
--   20260430160000_qualify_empty_search_path_rpc_bodies.sql
--     (process_payment_v8, apply_refund_to_payment×2, remove_order_item, void_payment)
--   20260430170000_qualify_cash_drawer_rpc_bodies.sql
--     (record_cash_operation)
--
-- This migration finishes the body rewrites for the remaining 8 broken
-- functions found by the table-ref audit:
--   public.broadcast_order_changes()                                  (trigger)
--   public.get_eod_cash_summary(uuid, date)
--   public.get_kds_tickets_v2(uuid, text[], uuid)
--   public.get_session_variance_analysis(uuid)
--   public.record_refund_items(uuid, jsonb)
--   public.record_refund_items(uuid, jsonb, boolean)
--   public.reset_category_item_to_level(uuid, uuid, uuid, uuid, integer)
--   public.upsert_category_item_override(uuid, uuid, uuid, uuid, numeric, numeric,
--                                        boolean, numeric, text, integer,
--                                        boolean, text, integer, numeric)
--
-- Bodies preserved verbatim, with `public.` inserted in front of every
-- previously-unqualified public table, type, or function reference.
-- Cross-schema calls (e.g. realtime.send) are unchanged.

-- ============================================================
-- 1. broadcast_order_changes (trigger)
-- ============================================================
...
-- ============================================================
-- 4. get_session_variance_analysis
-- ============================================================
...
-- ============================================================
-- 5. record_refund_items (2-arg overload)
-- ============================================================
...
-- ============================================================
-- 6. record_refund_items (3-arg overload)
-- ============================================================
CREATE OR REPLACE FUNCTION public.record_refund_items(
  p_reversal_id uuid,
  p_items jsonb,
...
-- ============================================================
-- 7. reset_category_item_to_level
-- ============================================================
...
-- ============================================================
-- 8. upsert_category_item_override
-- ============================================================
