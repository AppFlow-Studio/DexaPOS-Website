-- =====================================================================
-- Migration: calculate_order_totals_fast v7 — per-item-refund reconcile
-- =====================================================================
-- In-place CREATE OR REPLACE. Forks the 2026-07-06 v6 body VERBATIM and makes a
-- SINGLE change: suppress the payment-level custom_refund_balance when the order
-- has per-ITEM refunds, so amount_due (card) stays reconcilable with
-- cash_amount_due after refunding one item on a dual-priced order.
--
-- Why: a refunded item is already re-exposed at its OWN card AND cash price by
-- the unpaid-subtotal/tax blocks below (correctly dual-priced). But
-- custom_refund_balance reconstructs the refund's card-equivalent from the
-- whole-payment ratio (v_card_total_calc - v_effective_paid), which is wrong
-- when items have different card/cash spreads — it inflates v_unpaid_card_total
-- and makes amount_due diverge from cash_amount_due (reported: card $8.39 vs
-- cash $6.53; the cash side already used the v_effective_paid_cash path and was
-- correct). Gating it to 0 when refunded items exist lets the item-level card
-- total win. Mirrors the client calculator lib/order-calculator.ts (`hasItemRefunds`
-- gate). split-evenly never sets refunded_quantity, so this stays inert there.
--
-- Cross-check on staging (dfwqakoyittmrwbqvxgw) before prod: recalc a refunded
-- dual-priced order and confirm amount_due / cash_amount_due reconcile and match
-- luqra_transactions. Rollback re-applies v6.
--
-- Everything else below is v6 verbatim. Original v6 notes retained:
-- ---------------------------------------------------------------------
...
-- v7: suppress custom_refund_balance for per-item refunds (see header). A
-- refunded item is already dual-priced at the item level (re-exposed at its own
-- card AND cash price by the unpaid blocks above). The payment-level residual is
-- card-denominated and, when items have different spreads, inflates amount_due
-- and diverges it from cash_amount_due. The cash side below
-- (v_unpaid_cash_total_from_payments, via v_effective_paid_cash) is unaffected.
SELECT EXISTS (
    SELECT 1 FROM public.order_items
    WHERE order_id = p_order_id
      AND is_voided = false
      AND COALESCE(refunded_quantity, 0) > 0
) INTO v_has_item_refunds;

v_custom_refund_balance := CASE
    WHEN v_has_item_refunds THEN 0
    ELSE GREATEST(v_payment_based_due - v_unpaid_card_total, 0)
END;
