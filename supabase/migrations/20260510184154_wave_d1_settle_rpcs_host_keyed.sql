...
BEGIN
    ...
    IF FOUND THEN
        ...
        WHERE settlement_batch_id=v_batch_uuid AND status='captured' AND is_settled=false;
...
BEGIN
    ...
    IF v_batch.status NOT IN ('pending','settling','retry','failed') THEN
        RAISE EXCEPTION 'Batch % is in status %. Expected pending/settling/retry/failed.', p_batch_uuid, v_batch.status;
    END IF;
