-- =====================================================================
-- Wave A.2 — process_payment_v11 (forked from v10)
-- =====================================================================
-- Why: today the acquirer-assigned batch number from the Castles terminal
-- response (`txnBatchNo`, mapped to `castles_transaction.batchNumber`) is
-- written to `order_payments.dejavoo_batch_number`. That column is named
-- after the wrong processor and there is no per-payment indexed home for
-- the host batch identity. Wave A.1 added `acquirer` and documented the
-- existing `batch_number` column. This wave teaches the active payment
-- RPC to populate them.
--
-- Five normalized deltas vs v10 (exception list for verify_verbatim_diff):
--
--   1. New DECLAREs: v_acquirer, v_batch_number.
--
--   2. Castles branch sets v_acquirer := 'TSYS' (current acquirer for our
--      Castles fleet) and v_batch_number := castles_transaction.batchNumber.
--      Dejavoo branch sets v_batch_number := dejavoo_transaction.batchNumber.
--      v_acquirer stays NULL for Dejavoo until we have a reliable acquirer
--      mapping for that gateway (TODO: future wave). Cash payments keep
--      both NULL.
--
--   3. INSERT INTO order_payments column list and VALUES extend with
--      `acquirer` and `batch_number`. `dejavoo_batch_number` continues to
--      be populated unchanged as a one-release read-shim so existing
--      reports/queries don't break instantly.
--
--   4. v_result jsonb_build_object extends with `acquirer` and
--      `batch_number` so idempotent replay returns them.
--
--   5. _idempotency_claim / _idempotency_complete op string flips from
--      'process_payment_v10' -> 'process_payment_v11'. Separate cache
...
-- from 'process_payment_v10' to 'process_payment_v11' in a follow-up
-- commit. Until that flip lands, this RPC is dormant and v10 stays the
-- live path. v10 is intentionally NOT dropped.
--
-- Apply AFTER:
--   - wave_a1_order_payments_acquirer_and_batch_number.sql
--   - process_payment_v10_platform_fees.sql
--
-- Rollback: DROP FUNCTION public.process_payment_v11(...). v10 is
-- untouched so the rollback is a single DROP, no data restore needed.
-- =====================================================================
...
DECLARE
    ...
    -- v11 additions: provider-agnostic batch identity.
    v_acquirer text;
    v_batch_number text;
BEGIN
    -- v11 delta: idempotency op string is 'process_payment_v11'.
    ...
    -- BEGIN_VERBATIM (mirrors v10 body exactly; deltas marked v11)
    ...
    END IF;
     ...
     IF p_terminal_response ? 'dejavoo_transaction' THEN
        ...
        -- v11 delta: provider-agnostic batch identity. Dejavoo gateway can
        -- sit on top of multiple acquirers; leaving v_acquirer NULL until a
        -- future wave wires up the mapping. batch_number still populated.
    ...
    IF p_terminal_response ? 'castles_transaction' THEN
        ...
        -- v11 delta: Castles fleet currently runs on TSYS as the acquirer.
        -- Stamp it explicitly so reconciliation can group payments to the
        -- correct host batch on Luqra. dejavoo_batch_number stays populated
        -- as a one-release shim; downstream readers should prefer
        -- order_payments.batch_number.
        v_acquirer := 'TSYS';
        v_batch_number := v_dejavoo_batch_number;
    END IF;
 ...
 PERFORM log_payment_event(
    ...
    -- v11 delta: extend v_result with acquirer + batch_number so idempotent
    -- replay returns them for the client.
...
GRANT EXECUTE ON FUNCTION public.process_payment_v11(
  uuid, text, numeric, numeric, numeric, jsonb, uuid, jsonb,
  integer, integer, boolean, uuid, uuid, uuid
) TO authenticated;

COMMENT ON FUNCTION public.process_payment_v11 IS
  'Idempotent payment-processing RPC. Forks v10 to populate provider-agnostic acquirer + batch_number columns on order_payments (Castles->TSYS, Dejavoo acquirer left NULL pending mapping). dejavoo_batch_number kept populated as one-release shim. Idempotency op namespace: ''process_payment_v11''.';

-- =====================================================================
-- How to test (Wave A.2)
-- =====================================================================
-- 1. Apply against staging (dfwqakoyittmrwbqvxgw) ONLY after wave_a1.
-- 2. Smoke test from psql:
--      SELECT proname FROM pg_proc WHERE proname = 'process_payment_v11';
--    Expect a single row.
-- 3. End-to-end (with services/paymentService.ts flipped to v11 in a
--    follow-up commit):
--      a. Run a Castles-terminal sale on a staging device.
--      b. SELECT acquirer, batch_number, dejavoo_batch_number
--           FROM order_payments
--           WHERE id = <new_payment_id>;
--         Expect acquirer='TSYS', batch_number = host txnBatchNo,
--         dejavoo_batch_number = same value (shim).
--      c. SELECT batch_number, COUNT(*), SUM(amount)
--           FROM order_payments
--           WHERE acquirer='TSYS' AND created_at > now() - interval '1 day'
--           GROUP BY batch_number;
--         Compare to Luqra's batch totals for the same window — should match.
-- 4. Idempotency replay: send the same payment with the same
--    idempotency key twice; second call must return the cached jsonb
--    including the new 'acquirer' and 'batch_number' keys.
-- 5. Cash payment: confirm acquirer + batch_number are NULL (no terminal
--    response means neither block runs).
-- 6. Rollback path: DROP FUNCTION process_payment_v11(...) — v10 still
--    serves all live traffic until the paymentService.ts flip.
-- =====================================================================
