-- =====================================================================
-- process_payment_v15: item-payment fully-paid balance fallback (ORD-S2-0001)
-- =====================================================================
-- In-place CREATE OR REPLACE of process_payment_v15. SAME name + 14-arg
-- signature, idempotency op still 'process_payment_v15'. Chains off the
-- latest v15 (20260606183812_process_payment_v15_dp_fee_pct): body is
-- byte-identical to that migration EXCEPT the one-line fix in the
-- v_is_item_payment fully-paid branch below (so the dual_pricing_fee = pct ×
-- total fix is preserved, NOT reverted).
--
-- Symptom: a fully-collected mixed card+cash split-by-item order stayed
-- 'partial' with a ~1¢ cash residual on reopen (e.g. ORD-20260610-S2-0001).
-- Root cause: the v_is_item_payment fully-paid branch decided solely on
-- v_unpaid_items_count and lacked the balance-settled fallback the split branch
-- (Fix #2) and else branch (Fix #3) already carry. A mixed dual-price allocation
-- can leave one unit paid_quantity < quantity even after every dollar is
-- collected, so the branch returned false → status stuck 'partial', the
-- force-mark block was skipped, and cash_amount_due kept the residual.
-- Fix: fully paid when ALL item qty is covered OR both canonical card/cash
-- balances are settled (same predicate as the split/else branches). The
-- force-mark block then zeroes the stray quantities and residuals.
--
-- Rollback: rollback/20260616120000_process_payment_v15_item_payment_balance_fallback_rollback.sql
--   (CREATE OR REPLACEs v15 back to the dp_fee_pct body).
-- =====================================================================
--
-- --- inherited dp_fee_pct (20260606183812) notes ---
-- process_payment_v15: dual_pricing_fee = pct × full total_amount.
--
-- The original v15 install computed v_dual_pricing_fee as the literal
-- (v_subtotal_portion - v_cash_equivalent_subtotal_portion) delta. That
-- baked the per-item card/cash price spread into the fee — wrong for
-- merchants whose menu prices weren't derived from the location's
-- dual_pricing_percentage (e.g. Americano staged with $5.25 card / $4.25
-- cash = $1.00 spread vs. the configured 4%, producing $1.00 fees on
-- $5.72 sales instead of the expected $0.23).
--
-- Fix: read the location's dual_pricing_percentage (already loaded into
-- v_dpp on the line above) and compute the fee as pct × the same total
-- the row will be inserted with — (v_payment_total + tip). This matches
-- the _stamp_pricing_snapshot fallback trigger formula and the documented
-- semantics ("pct × full total_amount"). tip_fee block is unchanged.
--
-- No other lines in the function change.

CREATE OR REPLACE FUNCTION public.process_payment_v15(p_order_id uuid, p_payment_method text, p_amount numeric DEFAULT NULL::numeric, p_tip_amount numeric DEFAULT 0, p_amount_tendered numeric DEFAULT NULL::numeric, p_item_allocations jsonb DEFAULT NULL::jsonb, p_staff_id uuid DEFAULT NULL::uuid, p_terminal_response jsonb DEFAULT NULL::jsonb, p_split_count integer DEFAULT NULL::integer, p_split_portion_index integer DEFAULT NULL::integer, p_force_card_pricing boolean DEFAULT false, p_terminal_id uuid DEFAULT NULL::uuid, p_idempotency_key uuid DEFAULT NULL::uuid, p_station_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
    ...
    -- as â SC residual, not zero. We separately recount unpaid items
    ...
    -- Per-payment cashâcard scaling rounds independently, so SUM(original_amount)
    -- across N cash item-payments drifts off card_total by a few cents â that
...
BEGIN
    ...
    IF v_is_item_payment THEN
        ...
        -- distributes SC + tax-on-SC via items-ratio, preserving cash â¤ card
    ...
    ELSE
        v_dual_pricing_fee := ROUND(
            (COALESCE(v_payment_total, 0) + COALESCE(p_tip_amount, 0))
            * COALESCE(v_dpp, 0) / 100,
            2
        );
    END IF;
    ...
    -- items" â that case won't trip the residual-after check because
    ...
    ELSIF (v_is_split_payment AND v_is_last_portion)
       ...
       OR v_closes_with_items THEN
        -- Closes the order â allocate all leftover SC here.
    ...
    -- $3.71 ticket gave $2.28 change (= $5 â items+tax) instead of $1.29
    -- (= $5 â items+tax+SC). The SC then sat as cash_amount_due residual
    ...
    ELSIF v_is_item_payment AND v_service_charge_share > 0 THEN
        ...
        ELSE
            ...
            -- v_tax_portion = v_payment_total â v_subtotal_portion lumps SC in).
    ...
    -- ====== original_amount (card-equivalent) â drift-free on the closer ======
    ...
    -- by card_total/cash_total â but rounding each payment independently means
    ...
    -- pre-INSERT value (card_total â SUM(prior original_amount)), i.e. exactly
    ...
    ELSIF v_closes_with_items OR v_is_full_remaining
        ...
        -- so the per-payment cashâcard rounding drift cannot strand a residual
    ...
    -- of refunds â the actual money collected in each payment's own currency.
    IF v_use_cash_pricing THEN
    ...
    -- so a $15 cash payment shows $15 paid â NOT the $26.98 card-equivalent.
    ...
    -- outstanding directly from `<mode>_total â <mode>_effective_paid`, which
    ...
    -- v_cash_based_due are derived drift-free from <mode>_total â effective_paid
    ...
    -- v_total_card_paid / v_total_cash_paid with cross-mode ratio scaling â
