...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.settlement_batches WHERE payment_terminal_id=p_terminal_id AND status IN ('pending','settling')) THEN
        RAISE EXCEPTION 'A settlement is already in progress for terminal %. Wait or check for a stuck batch.', p_terminal_id;
    END IF;

    -- Loop: find oldest open host-keyed batch with eligible payments.
    -- Skip-and-close empties (a batch can drain to zero via voids/refunds
    -- between capture and settle, or be a historical backfill with no
    -- live payments). Capped iterations as a runaway guard.
    LOOP
        v_safety_iter := v_safety_iter + 1;
        IF v_safety_iter > 50 THEN
            EXIT;
        END IF;

        SELECT * INTO v_batch FROM public.settlement_batches
        WHERE payment_terminal_id=p_terminal_id AND status='open' AND acquirer IS NOT NULL AND batch_number IS NOT NULL
        ...
        -- Empty batch: auto-close and continue.
        UPDATE public.settlement_batches
        SET status='closed',
            closed_at=NOW(),
