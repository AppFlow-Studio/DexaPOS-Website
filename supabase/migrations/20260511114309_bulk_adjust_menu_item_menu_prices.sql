...
BEGIN
  ...
  base AS (
    SELECT ic.menu_item_id, ic.category_id, tl.location_id, mi.name,
  ...
  rounded AS (
    SELECT menu_item_id, category_id, location_id, name, old_price,
      CASE p_rounding
  ...
  upserted AS (
    INSERT INTO location_menu_item_overrides (location_id, menu_id, category_id, menu_item_id, custom_price, updated_at)
    SELECT v.location_id, p_menu_id, v.category_id, v.menu_item_id, v.new_price, now() FROM valid v
...
BEGIN
  ...
  IF p_operation = 'reset' THEN
    ...
    reset_rows AS (
      UPDATE location_menu_item_overrides lmio
         SET custom_delivery_price = NULL, updated_at = now()
        FROM locs
       WHERE lmio.location_id = locs.location_id
         AND lmio.menu_id = p_menu_id
         AND lmio.menu_item_id = ANY(p_item_ids)
    ...
    SELECT
      COALESCE(jsonb_agg(jsonb_build_object('item_id', dr.menu_item_id, 'name', mi.name, 'old_price', NULL, 'new_price', NULL)), '[]'::jsonb),
      COUNT(*) INTO v_changes, v_updated
      FROM distinct_reset dr JOIN menu_items mi ON mi.id = dr.menu_item_id;
  ...
  base AS (
    SELECT ic.menu_item_id, ic.category_id, tl.location_id, mi.name,
      COALESCE(lmio.custom_price, lcio.custom_price, ci_prices.custom_price, lio.custom_price, mi.price) AS card_price,
      lmio.custom_delivery_price AS old_delivery,
      CASE p_operation
        WHEN 'markup_pct' THEN COALESCE(lmio.custom_price, lcio.custom_price, ci_prices.custom_price, lio.custom_price, mi.price) * (1 + p_value / 100.0)
        WHEN 'markup_amt' THEN COALESCE(lmio.custom_price, lcio.custom_price, ci_prices.custom_price, lio.custom_price, mi.price) + p_value
        WHEN 'set_fixed' THEN p_value
      END AS raw_price
    FROM item_cats ic CROSS JOIN target_locs tl
    JOIN menu_items mi ON mi.id = ic.menu_item_id AND mi.merchant_id = p_merchant_id
  ...
  rounded AS (
    SELECT menu_item_id, category_id, location_id, name, old_delivery,
      CASE p_rounding
        WHEN 'cent' THEN ROUND(raw_price, 2)
        WHEN 'nickel_up' THEN CEIL(raw_price * 20) / 20.0
        WHEN 'ninety_nine_up' THEN CASE WHEN raw_price <= FLOOR(raw_price) + 0.99 THEN FLOOR(raw_price) + 0.99 ELSE FLOOR(raw_price) + 1.99 END
  ...
  valid AS (SELECT * FROM rounded WHERE new_price >= 0),
  upserted AS (
    INSERT INTO location_menu_item_overrides (location_id, menu_id, category_id, menu_item_id, custom_delivery_price, updated_at)
    SELECT v.location_id, p_menu_id, v.category_id, v.menu_item_id, v.new_price, now() FROM valid v
    ON CONFLICT (location_id, menu_id, menu_item_id)
    DO UPDATE SET custom_delivery_price = EXCLUDED.custom_delivery_price, updated_at = now()
    RETURNING menu_item_id
  ),
  applied AS (SELECT 1 AS x FROM upserted),
  distinct_items AS (SELECT DISTINCT ON (menu_item_id) menu_item_id, name, old_delivery, new_price FROM valid ORDER BY menu_item_id),
  totals AS (
    SELECT
      COALESCE(jsonb_agg(jsonb_build_object('item_id', di.menu_item_id, 'name', di.name, 'old_price', di.old_delivery, 'new_price', di.new_price)), '[]'::jsonb) AS changes_json,
      (SELECT count(DISTINCT menu_item_id) FROM valid)::int AS updated_count,
      ((SELECT count(DISTINCT menu_item_id) FROM rounded) - (SELECT count(DISTINCT menu_item_id) FROM valid))::int AS skipped_count
    FROM distinct_items di
  )
...
REVOKE ALL ON FUNCTION public.bulk_adjust_menu_item_menu_delivery_prices(uuid, uuid, uuid, uuid[], text, numeric, text, text) FROM public;
GRANT EXECUTE ON FUNCTION public.bulk_adjust_menu_item_menu_delivery_prices(uuid, uuid, uuid, uuid[], text, numeric, text, text) TO authenticated, service_role;;
