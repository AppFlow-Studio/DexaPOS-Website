-- [POS/Web] 86ing: fold per-location snooze into get_menu_with_categories.
-- Re-applies the current live definition of get_menu_with_categories verbatim
-- (from 20260625000000_restore_effective_availability_in_get_menu_with_categories,
-- preserving the modifier display_order + location_item_modifier_groups UNION
-- behavior) with three additions:
--   1. Item effective_availability gains a snooze AND term (lio.snoozed_until).
--   2. Modifier is_active gains a snooze AND term (lmio_mod.snoozed_until).
--   3. Raw snoozed_until surfaced on item + modifier for dashboard badges.
-- Auto-expiry is lazy: `snoozed_until <= now()` stops filtering on the next read.
-- Because storefront (app/sites/actions.ts) and OrderOut
-- (lib/orderout/transform-menu.ts) already gate on effective_availability,
-- 86ing propagates to both for free.
