...
BEGIN
    ...
    IF v_def IS NULL THEN
        RAISE EXCEPTION 'wave_h2: missing unique index uq_settlement_batches_host_key on settlement_batches.';
    END IF;

    IF v_def !~* 'UNIQUE INDEX' THEN
        RAISE EXCEPTION 'wave_h2: uq_settlement_batches_host_key is not UNIQUE. Definition: %', v_def;
    END IF;

    IF v_def !~* '\(payment_terminal_id, merchant_id, acquirer, batch_number\)' THEN
        RAISE EXCEPTION 'wave_h2: uq_settlement_batches_host_key column list drift. Definition: %', v_def;
    END IF;

    IF v_def !~* 'WHERE \(batch_number IS NOT NULL\)' THEN
        RAISE EXCEPTION 'wave_h2: uq_settlement_batches_host_key predicate drift. Definition: %', v_def;
    END IF;
END
$$;
...
-- wave_h2_lazy_link_index_pin enforces this contract at migration time.
DECLARE
    v_existing_id uuid;
