...
serve(async (req: Request) => {
  ...
  try {
    const { phone, message, reservation_id, template_key }: NotifyRequest =
      await req.json()
    ...
    if (!reservation_id || !message) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'bad_request',
          message: 'reservation_id and message required'
        }),
        {
    ...
    await supabase.rpc('record_reservation_sms_result', {
