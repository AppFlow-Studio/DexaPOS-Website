// Deno-compatible copy of /lib/messaging/receipt-template.ts.
// Keep these two files in sync — the source has zero Next.js imports so this
// can be updated verbatim. Used by the `send-receipt` Edge Function.
...
// Amount shown = sum of payment net (total_amount − refunded_amount) across
// captured/paid/(partially_)refunded rows. order.total_amount is the bill,
// which can diverge from collected when there are discounts, comps,
