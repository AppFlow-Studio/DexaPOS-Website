// Preserved from Dexa-POS PR #206, ef103afa613958778415f26c8de6cec30014d6c8.
// Shared deployed Edge receipt contract: itemized POS receipts + concise kiosk confirmation.
declare const Deno: { env: { get(name: string): string | undefined } };
const RECEIPT_BASE_URL = (Deno.env.get('RECEIPT_BASE_URL') || 'https://dexaposai.com/receipts').replace(/\/+$/, '');
type ReceiptOrder = {
...
export function renderReceiptText(
  order: ReceiptOrder,
  location: ReceiptLocation | null,
  opts: { confirmation?: boolean; receiptUrl?: string | null } = {}
): string {
  const orderNumber = order.display_number || order.order_number || '-'
  const businessName = location?.name || 'Receipt'
  ...
  const lines = [
    businessName,
    `Order ${orderNumber.startsWith('#') ? orderNumber : `#${orderNumber}`} - ${shortDate}`,
    ...itemLines,
    `Total: ${total}`,
  ...
  if (opts.confirmation) {
    const confirmLines = [`Order ${orderNumber.startsWith('#') ? orderNumber : `#${orderNumber}`} at ${businessName}`]
    const receiptToken = order.receipt_token?.trim()
    if (opts.receiptUrl) {
      confirmLines.push(`Receipt: ${opts.receiptUrl}`)
  ...
  if (opts.receiptUrl) lines.push(`Receipt: ${opts.receiptUrl}`)
  return lines.join('\n')
}
