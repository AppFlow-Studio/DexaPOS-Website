-- ============================================================================
-- Device Registry Foundation Validation Pack
-- Run after supabase/migrations/046_device_registry_foundation.sql
-- Safe to run in SQL editor: fixture writes are wrapped in a transaction/rollback
-- Split versions also exist:
--   1) 046_device_registry_foundation_01_schema_checks.sql
--   2) 046_device_registry_foundation_02_behavior_checks.sql
--   3) 046_device_registry_foundation_03_manual_rls_checks.sql
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) Schema smoke checks
-- ---------------------------------------------------------------------------
...
--   * operational link clearing on recall/repair states
--   * admin helper views
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Manual RLS verification prompts
-- - run each from a real authenticated context for that user type
-- ---------------------------------------------------------------------------

-- HQ session:
-- SELECT COUNT(*) FROM public.device_inventory;
-- SELECT COUNT(*) FROM public.device_assignments;

-- Carrier session:
-- SELECT DISTINCT merchant_id FROM public.device_inventory ORDER BY merchant_id;
-- SELECT * FROM public.device_inventory ORDER BY created_at DESC LIMIT 20;

-- Merchant owner/admin session:
-- SELECT DISTINCT merchant_id FROM public.device_inventory ORDER BY merchant_id;
-- SELECT * FROM public.device_notes ORDER BY created_at DESC LIMIT 20;
