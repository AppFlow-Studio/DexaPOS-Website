...
| [GAPS-2026-08-29-RESERVATIONS-BUSINESS-AUDIT.md](GAPS-2026-08-29-RESERVATIONS-BUSINESS-AUDIT.md) | 🟠 **Native reservations, read from a business angle** rather than a correctness one. The engineering holds up — concurrency, tenancy, timezones, guest-data isolation. What is missing is the operating layer around it: **no reminders at all**, no no-show protection, the merchant may never learn a booking arrived (`notify_emails` defaults empty, dashboard does not refresh live), and a host moving a booking never tells the guest. Ten capability gaps **B1–B10** and eleven uncovered edge cases **E1–E11**, each with file-level evidence and a three-item priority |
