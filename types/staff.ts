...
export const inviteStaffSchema = z.object({
    ...
    phone: z.string().regex(/^\+?1?\d{10,14}$/, 'Invalid phone number').nullable(),
    ...
    pin_code: z.string().regex(/^\d{4,6}$/, 'PIN must be 4-6 digits').nullable(),
    hourly_rate: z.number().positive().nullable(),
    employment_type: z.enum(['full-time', 'part-time', 'contractor']).nullable(),
}).refine(
    (data) => data.staff_type === 'clerk' ? !!data.email : true,
    { message: 'Email is required for Clerk users', path: ['email'] }
).refine(
    (data) => {
    ...
    { message: 'PIN code is required when not auto-generating', path: ['pin_code'] }
)
...
export type StaffActionResponse<T = unknown> = StaffActionSuccess<T> | StaffActionError
