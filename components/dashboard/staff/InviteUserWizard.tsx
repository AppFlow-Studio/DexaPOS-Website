...
export function InviteUserWizard({
    ...
    const handleSubmit = async () => {
        const formData: InviteStaffFormData = {
            ...
            auto_generate_pin: staffType === 'pos' ? autoGeneratePin : (enablePosAccess ? autoGeneratePin : false),
            pin_code: staffType === 'pos'
                ? (autoGeneratePin ? null : pinCode)
                : (enablePosAccess ? (autoGeneratePin ? null : pinCode) : null),
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                        <BottomSheetHeader className="border-b">
                            <BottomSheetTitle>
                                ...
                                {currentStep === 'details' && `Add ${staffType === 'clerk' ? 'dashboard user' : 'POS staff'}`}
                                {currentStep === 'role' && `Assign ${firstName || 'user'} a role`}
                                {currentStep === 'locations' && 'Assign to locations'}
                                {currentStep === 'pos_config' && 'POS configuration'}
                                {currentStep === 'review' && `Review and ${staffType === 'clerk' ? 'send invite' : 'create staff'}`}
                            </BottomSheetTitle>
                            <BottomSheetDescription>
                                {currentStep === 'type' && 'Select whether this person needs dashboard access or POS-only access.'}
                                {currentStep === 'details' && 'Enter the basic information for the new staff member.'}
                                {currentStep === 'role' && "A team member's role determines what they can see and do."}
                                {currentStep === 'locations' && 'Select the locations where this team member will have access.'}
                                {currentStep === 'pos_config' && 'Configure PIN and employment details for POS access.'}
                                {currentStep === 'review' && 'Review all details before proceeding.'}
                            </BottomSheetDescription>
                        </BottomSheetHeader>
                                ...
                                <div className="py-6 space-y-6">
                                    ...
                                    {currentStep === 'type' && (
                                        <div className="space-y-4">
                                            ...
                                            <RadioGroup value={staffType} onValueChange={(value) => setStaffType(value as StaffType)}>
                                                <div className="space-y-3">
                                                    ...
                                                    >
                                                        ...
                                                        <label htmlFor="type-clerk" className="flex-1 cursor-pointer">
                                                    ...
                                                    >
                                                        ...
                                                        <label htmlFor="type-pos" className="flex-1 cursor-pointer">
                                                            <div className="flex items-center gap-2">
                                                                <Lock className="h-4 w-4" />
                                                                <span className="font-medium">POS Staff Only</span>
                                                            </div>
                                                            <div className="text-sm text-muted-foreground mt-1">
                                                                PIN-based access to POS only. No email or dashboard access. Perfect for cashiers and shift workers.
                                                            </div>
                                                        </label>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-2">
                                                <Label htmlFor="email">
                                                    ...
                                                    {staffType === 'pos' && <span className="text-muted-foreground">(optional)</span>}
                                                </Label>
                                                <Input
                                                    id="email"
                                                    type="email"
                                                ...
                                                <div className="space-y-3">
                                                    {filteredRoles.map((role) => (
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <div className="font-medium">{role.name} <Badge variant="outline" className="text-xs">{role.code}</Badge></div>
                                                                <div className="text-sm text-muted-foreground mt-1">
                                    ...
                                    {currentStep === 'locations' && (
                                        <div className="space-y-4">
                                            <div className="text-sm text-muted-foreground">
                                                Select one or more locations where this team member will have access.
                                            </div>
                                            <div className="space-y-2">
                                                {locations.length === 0 ? (
                                    ...
                                    {currentStep === 'pos_config' && (
                                        <div className="space-y-6">
                                                ...
                                                <>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="flex items-center justify-between p-4 rounded-lg border bg-muted/30">
                                                            <div className="flex-1">
                                                                <Label htmlFor="enablePos" className="text-base font-medium">
                                                                    Enable POS Access
                                                                </Label>
                                                                <p className="text-sm text-muted-foreground mt-1">
                                                                    Allow this manager to access the POS system with a PIN
                                                                </p>
                                                            </div>
                                                            <Switch
                                                                id="enablePos"
                                            ...
                                            {(staffType === 'pos' || (staffType === 'clerk' && enablePosAccess)) && (
                                                <div className="space-y-4">
                                                    ...
                                                    {!autoGeneratePin && (
                                                        <div className="space-y-2">
                                                            <Label htmlFor="pinCode">Enter PIN (4-6 digits)</Label>
                                                            <Input
                                                                id="pinCode"
                                                                type="text"
                                                                placeholder="1234"
                                                    ...
                                                    {autoGeneratePin && (
                                                        <div className="rounded-lg border bg-muted/30 p-3">
                                                            <p className="text-sm text-muted-foreground">
                                                                A 4-digit PIN will be automatically generated and shown after creation
                                                            </p>
                                                        </div>
                                                    )}
                                                </div>
                                            ...
                                            {staffType === 'pos' && (
                                                <>
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="space-y-2">
                                                            <Label htmlFor="posHourlyRate">Hourly Rate (optional)</Label>
                                    ...
                                    {currentStep === 'review' && (
                                        <div className="space-y-6">
                                            {/* Staff Type Badge */}
                                            <div className="flex items-center gap-2">
                                                <Badge variant={staffType === 'clerk' ? 'default' : 'secondary'} className="gap-1">
                                                    {staffType === 'clerk' ? <Mail className="h-3 w-3" /> : <Lock className="h-3 w-3" />}
                                            ...
                                            <div>
                                                <div className="text-sm font-medium mb-2">
                                                    {staffType === 'clerk' ? 'Inviting' : 'Creating'} as {selectedRole?.name}
                                                </div>
                                                <div className="text-sm text-muted-foreground">
                                                ...
                                                <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
                                                    ...
                                                    <div className="space-y-2 text-sm">
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <span>{autoGeneratePin ? 'Auto-generated' : 'Custom PIN set'}</span>
                                                        </div>
                                                        ...
                                                        {hourlyRate && (
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <span>${parseFloat(hourlyRate).toFixed(2)}/hour</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                        ...
                        <BottomSheetFooter className="border-t flex flex-col items-center justify-between">
                            <div className="flex items-center justify-between w-full">
                                ...
                                <Button
                                    onClick={handleNext}
                                    disabled={!canGoNext() || createPOSStaff.isPending || inviteClerkStaff.isPending}
                                    className="gap-2"
                                >
