...
import { GripVertical, Save, RotateCcw, Layers, Loader2 } from "lucide-react";
...
function SortableGroupRow({ entry, index }: SortableGroupRowProps) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "flex items-center gap-3 rounded-2xl border-0 bg-muted/60 p-2.5 shadow-none transition-colors",
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 rounded-2xl border bg-card p-6">
        {groups.length === 0 ? (
          ...
          <>
            {hasChanges && (
              <div className="flex items-center justify-between rounded-2xl border-0 bg-muted/60 p-2 shadow-none">
                <p className="text-sm text-muted-foreground">
                  Order changed
                  {locationId && (
            ...
            >
              ...
              <DragOverlay>
                {activeEntry && (
                  <div className="flex items-center gap-3 rounded-2xl border bg-card p-2.5 shadow-lg">
                    <GripVertical className="h-4 w-4 text-muted-foreground" />
                    <Layers className="h-3.5 w-3.5 text-purple-600" />
