...
import { Search, Filter, Download, AlertCircle, Info, AlertTriangle, User, Clock, Tag, X } from 'lucide-react';
...
export default function AuditLogViewer() {
  ...
  const categories = ['all', 'authentication', 'team', 'inventory', 'financial', 'settings', 'pos', 'carrier', 'customer'];
  ...
  const formatAction = (action) => {
    return action.split('.').pop().split('_').map(word => 
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Track all activities across your organization</p>
      </div>
              ...
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${getCategoryColor(log.action_category)}`}>
                      {log.action_category}
                    </span>
                  ...
                  <div className="font-semibold text-gray-900 mb-1">
                    ...
                    {log.resource_name && (
                      <span className="text-gray-600 font-normal"> · {log.resource_name}</span>
                    )}
                  </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-sm px-3 py-1 rounded-full font-medium ${getCategoryColor(selectedLog.action_category)}`}>
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-500 mb-2">ACTOR</h3>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-xs text-gray-500 mb-1">Name</div>
                      <div className="font-medium">{selectedLog.actor_name}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500 mb-1">Email</div>
                      <div className="font-medium">{selectedLog.actor_email}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500 mb-1">Role</div>
                      <div className="font-medium">{selectedLog.actor_role}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500 mb-1">Organization</div>
                      <div className="font-medium">{selectedLog.organization_name}</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <div className="text-xs text-gray-500 mb-1">Name</div>
                        <div className="font-medium">{selectedLog.resource_name || 'N/A'}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {selectedLog.changes && (
                <div>
                  <h3 className="text-sm font-semibold text-gray-500 mb-2">CHANGES</h3>
                  <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-500 mb-2">TIMESTAMP</h3>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="font-medium">{new Date(selectedLog.created_at).toLocaleString()}</div>
                  <div className="text-sm text-gray-600 mt-1">{formatDate(selectedLog.created_at)}</div>
                </div>
              </div>
            </div>
          </div>
